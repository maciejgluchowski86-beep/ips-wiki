...
- JavaScript `String.replace` uses a function replacement when inserting TeX displays, because `$` in an ordinary replacement string is collapsed to a single `# Compilation procedure for IPS wiki edits
