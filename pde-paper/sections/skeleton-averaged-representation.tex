...
\begin{proof}
Both constructions give classical solutions of~\eqref{eq:hessian-forward}. On the common regime in which the C-prime solution also satisfies the small-class bound $\abs{\lambda z_*}\leq1/8$, uniqueness in~\cref{thm:deterministic-iteration} identifies the two solutions. In particular, whenever both stated theorem hypotheses are imposed together, the represented fixed point is the deterministic small solution.
\end{proof}
