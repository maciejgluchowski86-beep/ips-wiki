...
ordinary interaction times are distinct, and every maximal backward predecessor
chain of patches with outgoing terminal boundary reaches time~$0$ after finitely
many steps.
\end{lemma}
...
successful interaction before~$t$. The conditional product disintegration is
therefore
\begin{align*}
\CE{Z_t^\eta\ind\bb{L_{T,t}}}{\mathcal G_T}
&=
...
C\bb{\eta\bb{i(P)},P^{\uparrow t}}.
\end{align*}
For an end patch, the continuation factor in this display is exactly the
one-site Feynman--Kac expectation together with the local no-success event; its
identification with~$C(\eta(i(P)),P^{\uparrow t})$ is
\eqref{eq:end-patch-continuation}. Combining the display with the tower identity
above proves the pointwise version of
...
\end{equation*}
This is finite by polynomial growth.
