\subsection{Centered-moment order}
\label{sec:moment-order}
\label{subsec:moment-monotonicity}

We now prove part~\textup{(ii)} of Theorem~\ref{thm:patch-positivity-order}.
The idea is to write the generator in the centered-monomial basis and show that
all off-diagonal coefficients are nonnegative. The resulting system of
equations for centered moments is then cooperative: a centered moment can
receive contributions from the other centered moments only with nonnegative
coefficients {\color{red} Reader has no clue waht this means. Keep it simple, just say its a locally Metzler system of equations with a reference}. The standard positivity property of such Metzler systems implies
that nonnegative centered moments remain nonnegative. Applied to the signed
measure~$\nu-\mu$, this gives preservation of~$\preceq_*$.

The required generator calculation is local. If~$i\notin A$, the update at~$i$
leaves~$\chi_A^*$ unchanged. If~$i\in A$, then
\begin{equation*}
\lambda_i\bb{E_{i,\rho_i}-I}\chi_A^*
=
\lambda_i\bb{\rho_i-p_i^\star}\chi_{A\setminus\cb{i}}^*
-
\lambda_i\chi_A^*.
\end{equation*}
Expanding~$\lambda_i\rho_i$ and~$\lambda_i$ in monomials and summing over
$i\in A$ gives
\begin{equation*}
\Lcal\chi_A^*
=\sum_{i\in A}\Cb{
-\lambda_i(\mb 0)\chi_A^*
+
\sum_{S\subseteq N(i)}
\bb{
\widehat{\lambda_i\rho_i}(S)
-p_i^\star\widehat{\lambda_i}(S)
}
\chi_S\chi_{A\setminus\cb{i}}^*
+
\sum_{\substack{S\subseteq N(i)\\S\neq\vn}}
\bb{-\widehat{\lambda_i}(S)}
\chi_S\chi_A^*
}.
\end{equation*}
Part~\textup{(i)} gives the required signs. For every nonempty
$S\subseteq N(i)$,
\begin{equation*}
-\widehat{\lambda_i}(S)\geq0.
\end{equation*}
For the coefficient multiplying
$\chi_S\chi_{A\setminus\cb{i}}^*$, first take~$S=\vn$. Since
$p_i^\star\leq\rho_i(\mb 0)$,
\begin{equation*}
\widehat{\lambda_i\rho_i}(\vn)
-p_i^\star\widehat{\lambda_i}(\vn)
=
\lambda_i(\mb 0)\bb{\rho_i(\mb 0)-p_i^\star}
\geq0.
\end{equation*}
If~$S\neq\vn$ and~$\widehat{\lambda_i}(S)<0$, the definition of
$p_i^\star$ gives
\begin{equation*}
\widehat{\lambda_i\rho_i}(S)
-p_i^\star\widehat{\lambda_i}(S)
\geq0.
\end{equation*}
If~$\widehat{\lambda_i}(S)=0$, the second inequality in
part~\textup{(i)} gives~$\widehat{\lambda_i\rho_i}(S)\geq0$. Thus every
coefficient multiplying a product~$\chi_S\chi_B^*$ in the generator formula is
nonnegative.

These products also have nonnegative expansions in the centered basis. Indeed,
for finite~$S,B\subseteq\Lambda$,
\begin{equation*}
\chi_S\chi_B^*
=
\bb{\prod_{j\in S\cap B}\bb{1-p_j^\star}}
\sum_{R\subseteq S}
\bb{\prod_{j\in S\setminus R}p_j^\star}
\chi_{R\cup(B\setminus S)}^*.
\end{equation*}
To see this, use
$\eta(j)(\eta(j)-p_j^\star)=(1-p_j^\star)\eta(j)$ on~$S\cap B$, and then
expand each remaining factor
$\eta(j)=p_j^\star+(\eta(j)-p_j^\star)$ for~$j\in S$. Since
$0\leq p_j^\star\leq1$, every coefficient in the expansion is nonnegative.

After collecting equal centered monomials, we may therefore write
\begin{equation}
\label{eq:centered-generator-metzler}
\Lcal\chi_A^*
=
\sum_{B\Subset\Lambda}M(A,B)\chi_B^*,
\qquad
M(A,B)\geq0\quad\text{for }B\neq A.
\end{equation}
For each~$A$ there are only only finitely many~$B$ with~$M(A,B)\neq 0$. Now let~$\sigma$ be a finite signed
measure and set
\begin{equation*}
m_A(t)=(\sigma P_t)(\chi_A^*).
\end{equation*}
The backward equation and~\eqref{eq:centered-generator-metzler} give
\begin{equation*}
\frac{d}{dt}m_A(t)
=
\sum_{B\Subset\Lambda}M(A,B)m_B(t).
\end{equation*}
This is a locally finite linear system with a Metzler matrix. Its semigroup is
positive, so coordinatewise nonnegativity of~$m(0)$ implies coordinatewise
nonnegativity of~$m(t)$ for every~$t\geq0$. Taking~$\sigma=\nu-\mu$, the
assumption~$\mu\preceq_*\nu$ says exactly that~$m_A(0)\geq0$ for every finite
$A$. Hence
\begin{equation*}
\mu P_t\preceq_*\nu P_t
\qquad\text{for every }t\geq0,
\end{equation*}
which proves the main assertion of part~\textup{(ii)}.

% ============================================================================
% BLURB D. Immediate consequences and completion of Theorem B
%
% Reader question:
%   How do the remaining assertions of part (ii) follow?
%
% Job:
%   Apply the same cone preservation to a single initial law to conclude
%       \mu\in\mathcal M_* => \mu P_t\in\mathcal M_*.
%   Then derive ordinary monomial comparison from
%       \chi_A
%       = sum_{B\subseteq A}
%         (prod_{i\in A\setminus B}p_i^\star)\chi_B^*,
%   whose coefficients are nonnegative. Do not use the patch representation
%   for this consequence.
%
%   Finally note that every nonempty centered moment of
%   \mu_{\mb p^\star} is zero, hence
%       \mu_{\mb p^\star}\preceq_*\mu
%   for every \mu\in\mathcal M_*, giving the final comparison in the theorem.
%   End explicitly by saying that part (ii), and therefore
%   Theorem~\ref{thm:patch-positivity-order}, is proved.
%
% Keep out:
%   Do not restore the old product-profile corollary here unless it is later
%   judged independently useful.
% ============================================================================

\subsection{Pure-death comparison}
\label{subsec:pure-death-comparison}

% Theorem~\ref{thm:patch-positivity-order} is complete before this subsection
% begins. This is the first place where the centered-moment order and the patch
% representation need to be combined.
%
% Main job:
%   Introduce the centered expansion of end factors here, because it is needed
%   for the comparison of two patch representations. For every end patch P
%   based at i, use affinity and the definition of p_i^star to write
%       C(z,P)
%       = C(p_i^star,P) + kappa(P)(z-p_i^star),
%   with C(p_i^star,P) >= 0 and kappa(P) >= 0. Expanding the product over end
%   patches expresses its expectation as a sum of centered moments with
%   nonnegative coefficients. Hence it is nonnegative for mu in M_*.
%
%   Then prove the pure-death comparison itself: removing the
%   environment-independent 1->0 transitions preserves patch positivity and the
%   threshold profile, and increases the relevant bulk and end patch
%   contributions. The centered end-factor expansion allows the comparison to
%   be made term by term for every mu in M_* and every successful-interaction
%   skeleton before averaging.
%
%   Put the invariant-measure comparison here afterwards as a consequence.
%
% Keep out:
%   Do not present the centered end-factor expansion as part of the proof of
%   centered-moment order; that theorem has already been proved by the generator
%   calculation.
%
% Placeholder. Rewrite subsection blurb by blurb.
