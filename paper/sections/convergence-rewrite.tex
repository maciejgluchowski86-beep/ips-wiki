...
\end{equation*}
since patch positivity of the original spin system guarantees that every
term in the sum is non-positive
...
prefactor are both non-positive: the sum equals
...
assumption, and all summands are non-negative by
...
Indeed, for every~$S\subseteq N(i)\cap I$,
\begin{equation*}
...
\geq0.
\end{equation*}
For nonempty~$S\cup T$, the sign follows from
\eqref{eq:principal-coefficient-criterion} and
\eqref{eq:principal-threshold-profile}; for~$S=T=\vn$, it follows from
