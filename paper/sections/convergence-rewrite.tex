...
continuing~$P$ produces no successful interaction in~$(T,t]$. Appendix~\ref{app:no-late-continuation} proves that these events are conditionally independent, so
\begin{equation*}
...
\end{equation*}
For~$P$ based at~$i$, the same appendix gives
\begin{equation}
...
\end{align}
Thus each end-patch factor in the finite-horizon expression converges
exponentially to the corresponding factor in the limiting product. Since the
full end contribution contains a product over~$\mathcal E_T$, the individual
errors must be accumulated, and the terminal spins may be dependent under~$\mu$.
Lemma~\ref{lem:end-factor-relaxation} carries out this product comparison by a
centered expansion and a telescoping estimate. It gives
\begin{align*}
&\abs{
\CE{W_t^\mu\ind\bb{L_{T,t}}}{\mathcal G_T}
-
...
Because~$E_T^R$ is determined by~$\mathcal G_T$, multiplying by
$\ind(E_T^R)$ and taking expectations gives
\begin{align*}
&\abs{
 \E_A\Cb{W_t^\mu\ind\bb{E_T^R\cap L_{T,t}}}
 -
...
\end{align*}
For the finite-horizon term,~$B_T\leq B_T^\varepsilon$ and the same formula
...
\leq1.
\end{equation*}
For the accumulated end-patch term,~$\abs{\mathcal E_T}\leq\abs R$ on
