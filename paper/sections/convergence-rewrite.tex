...
first~$T$ units of dual time stays inside~$R$, so its contribution is unchanged
...
\begin{align}\label{eq:LTt_representation}
...
\end{align*}
For the first term, the inequality~$B_T\leq B_T^\varepsilon$ and the same formula as~\ref{eq:LTt_representation} for the system with pure deaths removed give
\begin{equation*}
\E_A\Cb{
 \ind\bb{E_T^R}B_T
 \CP{L_{T,t}}{\mathcal G_T}
...
$E_T^R$, so~\ref{eq:LTt_representation} gives
\begin{equation*}
\E_A\Cb{
 \ind\bb{E_T^R}B_T\abs{\mathcal E_T}
 \prod_{P\in\mathcal E_T}
...
Starting from~$A\subseteq R$, the dual therefore remains inside~$R$ during
these first~$T$ units of dual time. Fixing the spins outside~$R$ to~$0$ does not
change the contribution of such a realization. Proposition~\ref{prop:confined-interaction-identity} makes this precise. The confined part is the first~$T$ units of the dual evolution from~$A$, so it corresponds to the factor~$P_T$ acting on~$\chi_A$, while the remaining~$t-T$ units are unchanged. Hence
\begin{equation*}
...
The unrestricted patch representation gives
\begin{equation*}
\E_A\Cb{W_t^\mu}
=
(\mu P_t)(\chi_A)
...
For the limiting estimate, apply the same bound with~$\mu=\mu_{\mb 1}$ and
horizon~$u\geq T$:
\begin{equation*}
0
\leq
\E_A\Cb{W_u^{\mu_{\mb 1}}\ind\bb{(E_T^R)^c}}
\leq
\rho_A(T,R).
\end{equation*}
On~$\cb{\abs{\mathcal P}<\infty}$,
$W_u^{\mu_{\mb 1}}\to W$ as~$u\to\infty$, while~$W=0$ on the complementary
event. Fatou's lemma therefore gives
\begin{equation*}
0
\leq
\E_A\Cb{W\ind\bb{(E_T^R)^c}}
