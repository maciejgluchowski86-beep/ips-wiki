...
\qquad(i\in I).
\end{equation*}
We first verify that this restricted spin system is patch positive. We check this directly. For each~$i\in I$ and~$\vn\neq S\subseteq N(i)\cap I$ we have 
\begin{equation*}
\widehat{\lambda_i^{I,\xi}}(S)
=\sum_{T\subseteq N(i)\cap U}
  \widehat{\lambda_i}(S\cup T)\prod_{j\in T}\xi(j)\leq 0,
\end{equation*}
since patch positivity of the original spin system guarantees every term in the sum is non-negative (c.f.~\eqref{eq:principal-coefficient-criterion}). Similarly
\begin{align*}
\widehat{\lambda_i^{I,\xi}(\rho_i^{I,\xi}-\rho_i^{I,\xi}(\mb{0}))}(S)
&=\sum_{T\subseteq N(i)\cap U}
  \widehat{\lambda_i(\rho_i-\rho_i(\mb{0}^{U,\xi}))}(S\cup T)\prod_{j\in T}\xi(j) \\
  &=\sum_{T\subseteq N(i)\cap U}
  \widehat{\lambda_i(\rho_i-\rho_i(\mb{0}))}(S\cup T)\prod_{j\in T}\xi(j)\\
  &\qquad + \bb{\rho_i(\mb{0}) - \rho_i(\mb{0}^{U,\xi})} \sum_{T\subseteq N(i)\cap U} \lambda_i^{I,\xi}(S\cup T) \prod_{j\in T}\xi(j) \geq 0.
\end{align*}
The first sum has only non-negative terms. The second sum is non-positive, because as we just noted~$\lambda_i^{I,\xi}(S)\leq 0$ for~$S\neq \vn$ and 
\begin{equation*}
    \rho_i(\mb{0}^{U,\xi}) = FILL THIS IN \geq \rho_i(\mb{0}).
\end{equation*}
Therefore, by Theorem~\ref{thm:patch-positivity-order}~\textup{(i)} the restricted spin system is patch
positive. ARGUMENT FOR WHY~$(p^{I,\xi})^* \leq p^*$ IS MISSING AND WHY~$\rho_i(\mb{0}^{U,\xi})\geq \rho_i(\mb{0})$. THIS IS HOW YOU ARGUE FOR PRESERVATION OF~$\mathcal{M}_*^I$ WHICH ALSO NEEDS TO BE DEFINED.
% ============================================================================
