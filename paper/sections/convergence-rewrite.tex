...
the revealed trajectory as a boundary condition. Only the states at~$\partial I:=N_*(I)\setminus I\subseteq U$ affects this evolution.
...
extended arbitrarily from~$\partial I$ to~$U$, and their durations are
$t_1+\dots+t_n = t$, then
\begin{equation*}
...
Now we repeatedly apply~\eqref{eq:mass_leak_decomposition}:
\begin{align*}
    \CE{\chi_A^*(\eta_t)}{\mathcal{G}_U}&= \sum_{k=1}^n \alpha_I(t_k)\prod_{l=k+1}^n\bb{1-\alpha_I(t_l)} P_{t_1}^{I,\xi_1}\cdots P_{t_{k-1}}^{I,\xi_{k-1}}\bb{\chi_A^*}(\zeta_I)\\&\quad + \prod_{k=1}^n \bb{1-\alpha_I(t_l)} \nu_n(\chi_A^*)\\
    &\geq -\prod_{k=1}^n \bb{1-\alpha_I(t_l)}\\
    & FILL THE REST OR IMPROVE ESTIMATE ON alpha IF NEEDED
\end{align*}
Hence, for every~$A\Subset \Lambda$ we have
\begin{equation*}
\liminf_{t\to\infty}\inf_\eta P_t(\chi_A^*)(\eta) = \liminf_{t\to\infty}\inf_\eta \E_\eta\CE{\chi_A^*(\eta_t)}{\mathcal{G_U}}\geq0.
\end{equation*}
%  ============================================================================
