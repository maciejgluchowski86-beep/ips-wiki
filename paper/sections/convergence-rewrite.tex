...
contributions for~$\Lcal^\varepsilon$. By the proof of
Corollary~\ref{cor:pure-death-comparison}, removing the pure deaths leaves the
signed dual generator unchanged. We therefore realize the two systems with the
same dual process, successful-interaction skeleton, and patch families. For a
...
\ind\bb{\abs{\mathcal P}<\infty}.
\end{align*}
For every realized bulk patch and every~$\mu\in\mathcal M_*$, respectively,
\begin{equation*}
