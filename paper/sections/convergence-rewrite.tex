...
process, successful interactions, and patch families. For a probability
measure~$\nu$, define~$W_s^{\varepsilon,\nu}$ from~$W_s^\nu$ by replacing each
patch contribution~$C$ by~$C^\varepsilon$, and set
\begin{equation*}
W^\varepsilon
=
...
\ind\bb{\abs{\mathcal P}<\infty}.
\end{equation*}
The proof of Corollary~\ref{cor:pure-death-comparison} gives, for every
$\nu\in\mathcal M_*$ and~$s\geq0$,
\begin{equation}
\label{eq:death-removed-weight-comparison}
0\leq W_s^\nu\leq W_s^{\varepsilon,\nu},
\qquad
\E_A\Cb{W_s^{\varepsilon,\nu}}
=(\nu P_s^\varepsilon)(\chi_A)
\leq1.
\end{equation}

If a patch~$P$ ends at an outgoing successful interaction, its site is
...
We next trace a successful interaction at time~$u>0$ backwards to time~$0$.
Start with the patch at its source ending at~$u$. If the current patch begins
with an incoming interaction, take the patch at the source of that interaction
ending at the same time. If it begins with an outgoing interaction, that
interaction must be a birth because the source remains dual-active in the
current patch, so take the preceding patch at the same site. In either case the
predecessor ends at an outgoing interaction. The boundary times decrease
strictly, and local finiteness of the graphical construction forces the chain
to reach time~$0$ after finitely many steps. Reversing the resulting patches
$P_1,\ldots,P_n$, their intervals concatenate from~$0$ to~$u$. Hence
\begin{equation}
...
On~$L_{T,t}^c$, let~$u\in(T,t]$ be the first successful interaction after~$T$
...
\prod_{k=1}^n C^\varepsilon(P_k).
\end{equation*}
The remaining bulk factors and the terms in the centered end expansion are
compared exactly as in the proof of
Corollary~\ref{cor:pure-death-comparison}. Since~$\mu\in\mathcal M_*$, the
centered moments are nonnegative, and therefore
\begin{equation*}
...
\end{equation*}
On~$\cb{\abs{\mathcal P}=\infty}$ the left-hand side is zero. Taking
