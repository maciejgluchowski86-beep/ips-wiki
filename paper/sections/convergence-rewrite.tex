...
\end{equation*}
The comparison above gives~$0\leq B_T\leq B_T^\varepsilon$. Proposition~\ref{prop:no-late-continuation-identity} gives
\begin{align*}
...
\end{align*}
Lemma~\ref{lem:end-factor-relaxation} therefore bounds the conditional absolute difference by
\begin{align*}
...
Since~$E_T^R\in\mathcal G_T$, we may multiply this bound by~$\ind\bb{E_T^R}$ and take expectations. For the first term, the continuation identity for the system with pure deaths removed and~$B_T\leq B_T^\varepsilon$ give
\begin{align*}
&\E_A\Cb{
 \ind\bb{E_T^R}B_T
 \CP{L_{T,t}}{\mathcal G_T}
...
\leq1,
\end{align*}
where the last inequality is~\eqref{eq:death-removed-weight-comparison}. For the second term, on~$E_T^R$ we have~$\abs{\mathcal E_T}\leq\abs R$, while the infinite continuation identity gives
\begin{align*}
&\E_A\Cb{
