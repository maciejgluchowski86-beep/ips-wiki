...
\quad(i\in I).
\end{align*}
Here~$f$ is a function on~$\cb{0,1}^I$, viewed as a cylinder function
on~$\Omega$. Denote the restricted semigroup by~$P_t^{I,\xi}$.
...
\qquad(t\geq0).
\end{equation*}
This holds for every prescribed boundary configuration~$\xi$; the lemma
also gives preservation when the boundary changes finitely many times
on a bounded time interval. The restricted system is also patch positive.
