...
the revealed trajectory as a boundary condition. Only the states at~$\partial I:=N_*(I)\setminus I\subseteq U$ affect this evolution.
...
extended arbitrarily from~$\partial I$ to~$U$, and their durations are~$t_1,\ldots,t_n$ with~$\sum_{k=1}^n t_k=t$, then
\begin{equation*}
\CE{f(\eta_t)}{\mathcal{G}_U}=P_{t_1}^{I,\xi_1}\cdots P_{t_n}^{I,\xi_n}(f)(\eta),
\end{equation*}
for any~$f$ that depends only on sites in~$I$.

We apply~\eqref{eq:mass_leak_decomposition} on successive unit intervals.
Its proof uses rate bounds uniform in the boundary, so the same event
gives a part of weight~$a_I:=\alpha_I(1)\in(0,1)$ even if the boundary
changes during the interval. For the revealed boundary path~$b$, let
$Q^b_{s,t}$ denote the conditional transition kernel from~$s$ to~$t$,
given by the corresponding product of restricted semigroups. In
particular, for any probability measure~$\nu$ on~$\cb{0,1}^I$,
\begin{equation*}
\nu Q^b_{k-1,k}
=a_I\delta_{\zeta_I}+(1-a_I)\nu',
\end{equation*}
where~$\nu'$ is another probability measure. The first part remains
in~$\mathcal M_*^I$ under the subsequent evolution; we apply the
decomposition again to the remaining part. After~$m=\lfloor t\rfloor$
such steps, for every~$B\subseteq I$,
\begin{align*}
\CE{\chi_B^*(\eta_t)}{\mathcal G_U}
&=\sum_{k=1}^m a_I(1-a_I)^{k-1}
   Q^b_{k,t}(\chi_B^*)(\zeta_I)
  +(1-a_I)^m\nu_t^b(\chi_B^*)\\
&\geq-\norm{\chi_B^*}_\infty(1-a_I)^{\lfloor t\rfloor},
\end{align*}
where~$\nu_t^b$ is the remaining probability law, including its evolution
from~$m$ to~$t$. The term indexed by~$k$
is the part separated at time~$k$, after the first~$k-1$ steps left it
in the remainder. Each such term is nonnegative because~$Q^b_{k,t}$
preserves~$\mathcal M_*^I$. Taking~$B=A$ and averaging gives
\begin{equation*}
P_t(\chi_A^*)(\eta)
\geq-\norm{\chi_A^*}_\infty(1-a_I)^{\lfloor t\rfloor},
\end{equation*}
uniformly in~$\eta$. The right-hand side tends to zero exponentially,
