...
\textit{Blurb scaffold for the proof of
Corollary~\ref{cor:oriented-ergodicity}. The paragraphs below specify the
exposition and mathematical steps to develop; they are not a finished proof.}
...
% ============================================================================
Corollary~\ref{cor:oriented-ergodicity} asserts that orientability gives
convergence to the invariant measure~$\pi$ from every initial configuration,
and that uniform orientability gives a uniform exponential rate.
Theorem~\ref{thm:common-invariant-limit} already gives convergence uniformly
over~$\mathcal M_*$. By compactness and the Feller property, it therefore
...
of orientability and condition on the evolution in the autonomous
set~$U(A)$. The conditional dynamics on~$I$ then have prescribed boundary
conditions. Restriction preserves patch positivity and, with the original
centering profile, the class~$\mathcal M_*^I$ of laws with nonnegative
centered moments, even when the boundary changes with time. Uniform deaths
allow us to write the conditional law after one unit of time as a convex
combination with a positive weight in~$\mathcal M_*^I$, uniformly over the
initial state and boundary path. This part remains in~$\mathcal M_*^I$;
repeating the decomposition on the remaining part makes its weight tend
to zero. This proves the displayed bound and hence ergodicity.
For uniform orientability, a coupling estimate on growing finite sets
makes this preliminary waiting time quantitative and yields the
exponential rate.
...
% ============================================================================
\paragraph{Blurb B. Restriction with a fixed boundary.}
Pick a nonempty finite set~$A$ and take~$I=\bar A$ and~$U=U(A)$ from
orientability. For~$\xi\in\cb{0,1}^U$, let~$\eta^{U,\xi}$ agree with~$\xi$
on~$U$ and with~$\eta$ elsewhere. For~$f$ depending only on~$I$, define
\begin{equation*}
...
\quad(i\in I).
\end{equation*}
Autonomy of~$I\cup U$ makes this a generator on~$\cb{0,1}^I$;
denote its semigroup by~$P_t^{I,\xi}$. Set
$g_i=\lambda_i(\rho_i-p_i^\star)$ and define~$g_i^{I,\xi}$ by the same
substitution. Explain the coefficient identities explicitly:
\begin{align*}
\widehat{\lambda_i^{I,\xi}}(S)
&=\sum_{T\subseteq N(i)\cap U}
  \widehat{\lambda_i}(S\cup T)\prod_{j\in T}\xi(j)
\leq0
&&(\vn\neq S\subseteq N(i)\cap I),\\
\widehat{g_i^{I,\xi}}(S)
&=\sum_{T\subseteq N(i)\cap U}
  \widehat{g_i}(S\cup T)\prod_{j\in T}\xi(j)
\geq0
&&(S\subseteq N(i)\cap I).
\end{align*}
Each product is~$0$ or~$1$. Account for the changed all-zero baseline
$a_i^\xi=\rho_i^{I,\xi}(\mb0)\geq p_i^\star$ when verifying patch positivity:
for~$S\neq\vn$,
\begin{equation*}
\widehat{\lambda_i^{I,\xi}(\rho_i^{I,\xi}-a_i^\xi)}(S)
=
\widehat{g_i^{I,\xi}}(S)
-(a_i^\xi-p_i^\star)\widehat{\lambda_i^{I,\xi}}(S)
\geq0.
\end{equation*}
Conclude patch positivity by Theorem~\ref{thm:patch-positivity-order}(i),
and use its centered-moment argument to obtain
\begin{equation*}
\mathcal M_*^I P_t^{I,\xi}\subseteq\mathcal M_*^I,
\qquad
\mathcal M_*^I
=\cb{\mu:\mu(\chi_B^*)\geq0\text{ for every }B\subseteq I}.
\end{equation*}
Emphasize that this uses the original~$p_i^\star$ throughout.
