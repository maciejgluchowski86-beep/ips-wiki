...
\end{equation}
and let~$W_t^{\varepsilon,\mu}$ be analogous with~$C$ replaced by
...
pure deaths changes its contribution by an exponential factor
\begin{equation}
...
\end{align}
The proof boils down to handling contributions from three events:
$(E_T^R)^c$, $E_T^R\cap L_{T,t}^c$, and $E_T^R\cap L_{T,t}$.
