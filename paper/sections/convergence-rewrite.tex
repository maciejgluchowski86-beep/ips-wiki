...
\qquad(i\in I).
\end{equation*}
Set~$g_i=\lambda_i(\rho_i-p_i^\star)$ and
$g_i^{I,\xi}=\lambda_i^{I,\xi}(\rho_i^{I,\xi}-p_i^\star)$.
...
\end{align*}
Indeed, each product is~$0$ or~$1$, and the signs of the original
coefficients follow from
\eqref{eq:principal-coefficient-criterion} and
\eqref{eq:principal-threshold-profile}, including
$\widehat g_i(\vn)=\lambda_i(\mb0)(\rho_i(\mb0)-p_i^\star)\geq0$.

To verify patch positivity of the restricted system, write
$a_i^\xi=\rho_i^{I,\xi}(\mb0)$. The second inequality above at~$S=\vn$,
together with~$\lambda_i^{I,\xi}(\mb0)\geq\varepsilon>0$, gives
$a_i^\xi\geq p_i^\star$. Hence, for nonempty~$S\subseteq N(i)\cap I$,
\begin{equation*}
...
\geq0.
\end{equation*}
Theorem~\ref{thm:patch-positivity-order}~\textup{(i)} therefore gives patch
positivity. Moreover, the signs of~$\widehat{\lambda_i^{I,\xi}}$ and
$\widehat{g_i^{I,\xi}}$ are precisely those used in
Section~\ref{subsec:moment-monotonicity} to prove preservation of
nonnegative centered moments. Applying that argument with the original
profile~$(p_i^\star)_{i\in I}$ gives
\begin{equation*}
...
\end{equation*}
where~$\mu$ ranges over probability measures on~$\cb{0,1}^I$.
