...
% ============================================================================
We next show that, after any positive time, a fixed positive part of the
law belongs to~$\mathcal M_*^I$, regardless of the initial configuration
and boundary conditions. Patch positivity gives
$\lambda_i(\rho_i-p_i^\star)\geq0$, and the pure-death assumption gives
$\lambda_i(1-\rho_i)\geq\varepsilon$. Hence
\begin{equation*}
\lambda_i\rho_i\geq p_i^\star\lambda_i\geq\varepsilon p_i^\star,
\qquad
\lambda_i(1-\rho_i)\geq\varepsilon(1-p_i^\star).
\end{equation*}
We can therefore separate from the dynamics, at each site~$i$, an
independent rate-$\varepsilon$ clock that refreshes the spin with a
Bernoulli~$p_i^\star$ coin. Write
$\mu_{\mb p^\star}^I=\bigotimes_{i\in I}\mathrm{Bernoulli}(p_i^\star)$.
Every nonempty centered moment of this product measure is zero, so
$\mu_{\mb p^\star}^I\in\mathcal M_*^I$.

The remaining updates can be generated using independent Poisson clocks
of rate~$\overline\lambda=\sup_{i,\eta}\lambda_i(\eta)$ at each site,
with suitable acceptance probabilities. During an interval of length~$s>0$, consider the event that every
rate-$\varepsilon$ clock in~$I$ rings at least once and none of the
remaining clocks rings. On this event, the final spins are independent
Bernoulli~$p_i^\star$ variables, since they are given by the last refresh
at each site. Thus, for every initial configuration~$x\in\cb{0,1}^I$
and boundary~$\xi$,
\begin{align*}
P_s^{I,\xi}(x,\cdot)
&=\alpha_I(s)\mu_{\mb p^\star}^I+(1-\alpha_I(s))\nu,\\
\alpha_I(s)
&=e^{-\overline\lambda\abs I s}(1-e^{-\varepsilon s})^{\abs I}>0,
\end{align*}
for some probability measure~$\nu$. The same event and bound apply if
the prescribed boundary changes during the interval. In particular,
every unit interval contributes a part of weight~$a_I=\alpha_I(1)>0$
in~$\mathcal M_*^I$. Since this class is preserved by the subsequent
evolution, we can repeat the decomposition on the remaining part of
the law.
...
\begin{equation*}
Q^b_{k-1,k}(x,\cdot)=a_I\mu_{\mb p^\star}^I+(1-a_I)R_k^b(x,\cdot).
\end{equation*}
For~$m=\lfloor t\rfloor$, expand according to the first extracted part:
\begin{align*}
\delta_{\eta|_I}Q^b_{0,t}
&=\sum_{k=1}^m a_I(1-a_I)^{k-1}\mu_{\mb p^\star}^I Q^b_{k,t}
 +(1-a_I)^m\nu_t^b,\\
\nu_t^b
&=\delta_{\eta|_I}R_1^b\cdots R_m^b Q^b_{m,t}.
\end{align*}
The measure~$\nu_t^b$ is a probability measure, and every term
$\mu_{\mb p^\star}^I Q^b_{k,t}$ belongs to~$\mathcal M_*^I$. The target is therefore
\begin{equation*}
