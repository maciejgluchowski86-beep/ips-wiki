...
&= \bb{P_{s_1^1}^{I,\xi_1^1}\cdots P_{s_{n_1}^1}^{I,\xi_{n_1}^1}}\cdots \bb{P_{s_1^{\lfloor t\rfloor}}^{I,\xi_1^{\lfloor t\rfloor}}\cdots P_{s_{n_{\lfloor t\rfloor}}^{\lfloor t\rfloor}}^{I,\xi_{n_{\lfloor t\rfloor}}^{\lfloor t\rfloor}}}(f)(\eta),
\end{align*}
gives a subdecomposition with~$s_1^k+\dots+s_{n_k}^k = 1$ for~$k < \lfloor t\rfloor$ and~$s_1^{\lfloor t\rfloor}+\dots +s_{n_{\lfloor t\rfloor}}^{\lfloor t\rfloor}\geq 1$.
We apply~\eqref{eq:mass_leak_decomposition} on successive unit intervals
\begin{align*}
\CE{\chi_A^*(\eta_t)}{\mathcal G_U}
&=\sum_{k=1}^{\lfloor t\rfloor} a_I(1-a_I)^{n-k}
   \bb{P_{s_1^1}^{I,\xi_1^1}\cdots P_{s_{n_1}^1}^{I,\xi_{n_1}^1}}\cdots \bb{P_{s_1^{k-1}}^{I,\xi_1^{k-1}}\cdots P_{s_{n_{k-1}}^{k-1}}^{I,\xi_{n_{k-1}}^{k-1}}}(\chi_A^*)(\zeta_I)\\
  &\quad +(1-a_I)^{\lfloor t\rfloor}\nu_t(\chi_A^*)\\
&\geq-\norm{\chi_A^*}_\infty(1-a_I)^{\lfloor t\rfloor},
\end{align*}
where~$\nu_t$ is a probability measure. The sum is non-negative because~$\delta_{\zeta_I}\in\mathcal{M}_*^I$ and each~$P_s^{I,\xi}$ preserves~$\mathcal{M}_*^I$. 
...
% ============================================================================
To obtain convergence to~$\pi$, we combine the preceding moment bound
with Theorem~\ref{thm:common-invariant-limit} after a further fixed time.
...
$\delta_{\eta_n}P_{t_n-s}\Rightarrow\mu$.
For every finite~$B\subseteq\Lambda$, the preceding bound implies
\begin{equation*}
\mu(\chi_B^*)
...
configurations for every local~$f$. Integrating against any initial
law~$\nu$ gives~$\nu P_t\Rightarrow\pi$; taking~$\nu$ invariant shows
that~$\pi$ is the unique invariant probability measure.

The finite-volume estimate also gives eventual entry into
$\mathcal M_{-,1}^I
=\cb{\mu:(\mu+\delta_{\mb1_I})/2\in\mathcal M_*^I}$.
...
-\norm{\chi_B^*}_\infty\bb{1-\alpha_I(1)}^{\lfloor t\rfloor}
\geq0
\end{equation*}
for all sufficiently large~$t$. Since~$I$ is finite, one time works
for all~$B\subseteq I$, uniformly in~$\eta$. This entry time may depend
on~$I$.
