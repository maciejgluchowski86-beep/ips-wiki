...
%
% Reader question:
%   Why does boundary substitution preserve the sign conditions?
% Role in the proof:
%   Define the finite system and establish preservation at the ORIGINAL
%   profile p^star, even though the restricted patch threshold may differ.
% Drafting target:
%   Two short paragraphs around the substitution identities. Distinguish the
%   generator L from its semigroup P. No conditioning is used at this stage.
% ============================================================================
Fix a nonempty finite set~$A$ and take~$I=\bar A$ and~$U=U(A)$ as in
orientability property. For~$\xi\in\cb{0,1}^U$, let~$\eta^{U,\xi}$ agree with~$\xi$
on~$U$ and with~$\eta$ elsewhere. For a function~$f$ on~$\cb{0,1}^I$,
viewed as a cylinder function on~$\Omega$, define
\begin{equation*}
\Lcal^{I,\xi}f(\eta)=\Lcal f(\eta^{U,\xi}).
\end{equation*}
Since~$I\cup U$ is autonomous, this defines a spin-system generator
on~$\cb{0,1}^I$, with semigroup~$P_t^{I,\xi}$ and rates
\begin{equation*}
...
\qquad(i\in I).
\end{equation*}
We first verify that this restricted spin system is patch positive.
For each~$i\in I$ and~$\vn\neq S\subseteq N(i)\cap I$ we have
\begin{equation*}
\widehat{\lambda_i^{I,\xi}}(S)
=\sum_{T\subseteq N(i)\cap U}
  \widehat{\lambda_i}(S\cup T)\prod_{j\in T}\xi(j)\leq0,
\end{equation*}
since patch positivity of the original spin system guarantees that~$\widehat{\lambda_i}(S)\leq 0$ whenever~$S\neq \vn$ 
(see~\eqref{eq:principal-coefficient-criterion}). Similarly,
\begin{align*}
\widehat{\lambda_i^{I,\xi}(\rho_i^{I,\xi}-\rho_i^{I,\xi}(\mb0))}(S)
&=\sum_{T\subseteq N(i)\cap U}
  \widehat{\lambda_i(\rho_i-\rho_i(\mb0^{U,\xi}))}(S\cup T)
  \prod_{j\in T}\xi(j)\\
&=\sum_{T\subseteq N(i)\cap U}
  \widehat{\lambda_i(\rho_i-\rho_i(\mb0))}(S\cup T)
  \prod_{j\in T}\xi(j)\\
&\quad+\bb{\rho_i(\mb0)-\rho_i(\mb0^{U,\xi})}
  \sum_{T\subseteq N(i)\cap U}
  \widehat{\lambda_i}(S\cup T)\prod_{j\in T}\xi(j)
\geq0.
\end{align*}
The first sum has only non-negative terms. The second sum and its
prefactor are both non-positive: we again use
$\widehat{\lambda_i^{I,\xi}}(S)\leq0$ for~$S\neq\vn$, and
\begin{equation*}
\rho_i(\mb0^{U,\xi})
=\rho_i(\mb0)
+\frac{1}{\lambda_i(\mb0^{U,\xi})}
 \sum_{\vn\neq T\subseteq N(i)\cap U}
 \widehat{\lambda_i(\rho_i-\rho_i(\mb0))}(T)
 \prod_{j\in T}\xi(j)
\geq\rho_i(\mb0).
\end{equation*}
Here the denominator is at least~$\varepsilon$ by the pure-death
assumption, and the terms in the sum are non-negative by
\eqref{eq:principal-coefficient-criterion}.
Therefore, by Theorem~\ref{thm:patch-positivity-order}~\textup{(i)},
the restricted spin system is patch positive.

Its patch threshold satisfies~$(p_i^{I,\xi})^\star\leq p_i^\star$.
To verify this first we note, that for every~$S\subseteq N(i)\cap I$,
\begin{equation*}
...
\geq0.
\end{equation*}
Indeed, for terms with~$S\cup T\neq\vn$, the non-negativity follows from
\eqref{eq:principal-coefficient-criterion} and
\eqref{eq:principal-threshold-profile}. For the~$S=T=\vn$ term, it follows from
$p_i^\star\leq\rho_i(\mb0)$. In particular,
\begin{equation*}
\frac{\widehat{\lambda_i^{I,\xi}\rho_i^{I,\xi}}(S)}
     {\widehat{\lambda_i^{I,\xi}}(S)}
\leq p_i^\star
\qquad\text{whenever }\widehat{\lambda_i^{I,\xi}}(S)<0,\ S\neq\vn,
\end{equation*}
so~\eqref{eq:principal-threshold-profile} gives the threshold
comparison. 

Let~$\mathcal M_*^I$ be the equivalent of~$\mathcal{M}_*$ for the restricted process,
\begin{equation*}
    \mathcal{M}_*^I = \cb{\mu\in \mathcal{P}\bb{\cb{0,1}^I}: \mu\bb{\chi_B^*}\geq 0 \text{ for all }B\subseteq I}.
\end{equation*}
The restricted process preserves~$\mathcal{M}_*^I$.  Preservation at this centering uses both bounds
\begin{equation*}
(p_i^{I,\xi})^\star\leq p_i^\star
\leq\rho_i(\mb0)\leq\rho_i^{I,\xi}(\mb0).
\end{equation*}
The first gives the nonnegative nonconstant coefficients of
$\lambda_i^{I,\xi}(\rho_i^{I,\xi}-p_i^\star)$, and the last bounds give its
nonnegative constant coefficient. To see preservation directly, choose
$h>0$ with~$h\sum_{i\in I}\lambda_i^{I,\xi}(\mb0)\leq1$. For~$B\subseteq I$,
\begin{align*}
...
\end{align*}
Both~$\lambda_i^{I,\xi}(\rho_i^{I,\xi}-p_i^\star)$ and
...
\end{equation*}
the right-hand side is therefore a nonnegative linear combination of
centered monomials. The same holds after any number of applications of
$\mathrm{Id}+h\Lcal^{I,\xi}$. Consequently, for~$\mu\in\mathcal M_*^I$,
\begin{equation*}
(\mu P_t^{I,\xi})(\chi_B^*)
=\lim_{n\to\infty}
...
\qquad(B\subseteq I),
\end{equation*}
which proves~$\mathcal M_*^I P_t^{I,\xi}\subseteq\mathcal M_*^I$.
...
by~$Q^b_{s,t}$. Finite composition gives preservation of~$\mathcal M_*^I$.
