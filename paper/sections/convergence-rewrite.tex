...
\end{equation*}
so~\eqref{eq:principal-threshold-profile} gives the claimed threshold
comparison. Let~$\mathcal M_*^I$ be the class of probability measures~$\mu$
on~$\cb{0,1}^I$ such that~$\mu(\chi_B^*)\geq0$ for every~$B\subseteq I$,
with~$\chi_B^*$ centered at the original~$p_i^\star$.
Preservation at this centering uses both bounds
\begin{equation*}
