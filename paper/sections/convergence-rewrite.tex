...
\end{equation*}
The comparison above gives~$0\leq B_T\leq B_T^\varepsilon$. Proposition~\ref{prop:no-late-continuation-identity} gives {\color{red} sentence about how on the event~$L_{T,t}$ all the bulk and end patches never change from time T onward, except the end patches get longer. Also figure for the event is missing, it previously existed}
\begin{align*}
...
\end{align*}
Lemma~\ref{lem:end-factor-relaxation} {\color{red} Doesnt tell the story that the event~$L_{T,t}$ factorizes per site and the appropriate terms converge pointise to~$C\bb{\rho_{i(P)}(\mb 0),P}$} bounds the conditional absolute difference by {\color{red} difference of what???}
\begin{align*}
...
Since~$E_T^R\in\mathcal G_T$, we may multiply this bound by~$\ind\bb{E_T^R}$ {\color{red} awful prose, describing steps instead of showing them} and take expectations. For the first term, the continuation identity {\color{red} completely unclear what "continuation identity" means. abnormal language} for the system with pure deaths removed and~$B_T\leq B_T^\varepsilon$ give
\begin{align*}
&\E_A\Cb{
 \ind\bb{E_T^R}B_T
 \CP{L_{T,t}}{\mathcal G_T}
...
\leq1,
\end{align*}
where the last inequality is~\eqref{eq:death-removed-weight-comparison}. For the second term {\color{red} reader is lost}, on~$E_T^R$ we have~$\abs{\mathcal E_T}\leq\abs R$, while the infinite continuation identity gives
\begin{align*}
&\E_A\Cb{
...
\leq\abs R.
\end{align*}
The last inequality follows from~$0\leq W\leq W^\varepsilon$ and~\eqref{eq:limiting-death-removed-mass}. Combining the two bounds proves~\eqref{eq:no-late-relaxation-bound}. 
\end{proof}
