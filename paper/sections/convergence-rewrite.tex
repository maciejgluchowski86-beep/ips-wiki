...
process, successful interactions, and patch families {\color{red} I think its proven in the appendix/earlier, but the reader doesnt know this}. For a probability
measure~$\mu$, define
\begin{align*}
W_t^{\varepsilon,\mu}
&=
...
$\mu\in\mathcal M_*$ and~$t\geq0$,
\begin{equation}
\label{eq:death-removed-weight-comparison}
0\leq W_t^\mu\leq W_t^{\varepsilon,\nu},
\qquad
...
We now show that for a successful interaction happening at time~$u>0$ there exists a trail of~$\mathsf{XO}$ type patches that spans time from~$0$ to~$u$. Every successful interaction, except the formal initial interaction~$\iota_0$, ends a~$\mathsf{XO}$ patch. We call the successful interaction that starts this patch the ancestor of the previous successful interaction. Jumping from ancestor to ancestor generates a finite sequence of successful interactions ending at~$\iota_0$. Indeed, the boundary times decrease
...
Corollary~\ref{cor:pure-death-comparison} {\color{red} write explicitly that~$C(P)\leq C^\varepsilon(P)$ and the more complicated inequality for~$\mu(...)$, and then say the proof is in proof of corollary 3.1. This reads as incomplete and even unclear to the reader}. Since~$\mu\in\mathcal M_*$, the
centered moments are nonnegative {\color{red} This should be subsumed by earlier note}, and therefore
\begin{equation*}
