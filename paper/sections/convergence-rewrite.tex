...
%
% Reader question:
%   What event produces a law in M_*^I regardless of the initial state?
% Role in the proof:
%   Obtain a positive mixture weight uniform in the initial state and boundary.
% Drafting target:
%   Explain the independent refreshes, then the event and its probability.
%   Use the product law at p^star in the repeated decomposition in Blurb D.
% ============================================================================
...
We verify that the restricted process has positive probability of entering~$\zeta_I$ after any positive time. The rate at which a site turns to state~$1$ is~$\lambda_i\rho_i  \geq \varepsilon p_i^*$, which is positive if~$p_i^*>0$. Similarly, the rate at which a site turns to state~$0$ is~$\lambda_i\bb{1 - \rho_i}\geq \varepsilon \bb{1-p_i^*}$, which is positive if~$p_i^* = 0$. Both of these inequalities follow from patch positivity. Thus every site~$i\in I$ has uniformly positive rate at which it takes the state~$\zeta_I(i)$. The event that after time one every site in~$I$ experienced at least one update, and at the last one its state aligned with~$\zeta_I$ is thus uniformly positive in~$\eta$ and boundary conditions, and we denote it~$\alpha_I$.
...
configurations for every local~$f$ and thus ergodicity.

Now we prove that the result can be strenghtened to uniform exponential ergodicity, if the lattice is uniformly orientable. First note that on finite~$I$ the evolution enters
$\mathcal M_{-,1}^I
=\cb{\mu:(\mu+\delta_{\mb1_I})/2\in\mathcal M_*^I}$ in finite time, uniformly in initial configuration.
Indeed, uniform deaths imply~$p_i^\star<1$, and for every
nonempty~$B\subseteq I$,
\begin{equation*}
\bb{(\delta_\eta P_t)|_I+\delta_{\mb1_I}}(\chi_B^*)
\geq
\prod_{i\in B}(1-p_i^\star)
-\norm{\chi_B^*}_\infty\bb{1-\alpha_I}^{\lfloor t\rfloor}
\geq0
\end{equation*}
for all sufficiently large~$t$. Since~$I$ is finite, a fixed time works
for all~$B\subseteq I$, uniformly in~$\eta$. This entry time may however depend
on~$I$.
...
% Exposition:
%   Choose the region, compare with a measure in M_*, and apply Theorem C.
%   The clock estimate is proved in the appendix.
% ============================================================================
Uniform orientability gives a quantitative comparison with~$\mathcal M_*$
on sets large enough to control the subsequent evolution.
Let~$h,r$ be as in Definition~\ref{def:orientability}, and fix~$f$
supported on a finite nonempty set~$A$. Choose the speed~$v$ in
Lemma~\ref{lem:finite-propagation} for error~$e^{-\varepsilon s/2}$;
as its proof shows,~$v$ is independent of~$A$. For~$s\geq0$, set
\begin{gather*}
H_s=\max_{i\in A}h(i)+r\lceil vs\rceil,
\qquad U_s=\cb{i:h(i)>H_s},\\
I_s=\bb{\bigcup_{n\geq0}B(A,n)}\cap\cb{i:h(i)\leq H_s},
\qquad
\ell_s=H_s-\min_{i\in A}h(i)+1.
\end{gather*}
The sets~$U_s$ and~$I_s\cup U_s$ are autonomous. Since each step along
a neighbourhood edge raises the height by at least~$1$ and at most~$r$,
$I_s\subseteq B(A,H_s-\min_A h)$ and
\begin{equation*}
...
Condition on the trajectory in~$U_s$ and couple two copies of the
restricted system, one from~$\eta|_{I_s}$ and one from~$\mb1_{I_s}$,
using the same clocks and marks. Agreement becomes permanent from
greater to smaller heights: once all neighbours of a site agree,
its next pure-death event makes the two copies agree there as well.
Lemma~\ref{lem:oriented-finite-coupling} in
Appendix~\ref{app:oriented-finite-coupling} quantifies this argument.
The law of the second copy at time~$u\geq0$ belongs to~$\mathcal M_*^{I_s}$ by
Lemma~\ref{lem:boundary-moment-preservation}. Average that law over the
boundary trajectory and extend it by all~$1$'s outside~$I_s$.
The resulting measure~$\mu$ belongs to~$\mathcal M_*$, since its centered
moments factor into a nonnegative moment on~$I_s$ and factors
$1-p_i^\star$ outside. Writing~$d=\max\cb{1,\sup_i\abs{N(i)}}$, the
coupling estimate gives, for every~$g$ supported on~$I_s$,
\begin{equation*}
\abs{P_ug(\eta)-\mu(g)}
\leq2\norm g_\infty\abs{I_s}(2d)^{\ell_s}e^{-\varepsilon u/2}.
\end{equation*}
Here~$\mu$ may depend on~$\eta,u,s$, while the bound is uniform in~$\eta$.

We can now run the original process for a further time~$s$.
Using finite propagation to replace~$P_sf$ by~$P_s^{I_s,0}f$, and then
Theorem~\ref{thm:common-invariant-limit}, gives
\begin{align*}
\abs{P_{u+s}f(\eta)-\pi(f)}
&\leq2\norm{P_sf-P_s^{I_s,0}f}_\infty\\
&\quad+\abs{(\delta_\eta P_u-\mu)(P_s^{I_s,0}f)}
       +\abs{(\mu P_s)(f)-\pi(f)}\\
&\leq C_f(1+s)^D
 \bb{e^{-\varepsilon s/2}
       +e^{rv\log(2d)s-\varepsilon u/2}}.
\end{align*}
Take~$u=\kappa s$, where
$\kappa=1+2rv\log(2d)/\varepsilon$.
Both exponential terms then equal~$e^{-\varepsilon s/2}$.
Absorbing the polynomial factor and writing~$t=(1+\kappa)s$ proves
\begin{equation*}
\sup_\eta\abs{P_tf(\eta)-\pi(f)}
\leq C_f e^{-\gamma t},
\qquad
\gamma=\frac{\varepsilon}{4(1+\kappa)}>0.
\end{equation*}
The rate~$\gamma$ is independent of~$f$, as required.
\par
\end{proof}
\endgroup
