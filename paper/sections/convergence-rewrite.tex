...
Fix the successful-interaction skeleton up to time~$T$, and put~$h=t-T$. For
...
\rho_i(\mb 0)
+
e^{-\lambda_i(\mb 0)h}\bb{z-\rho_i(\mb 0)}.
\end{equation*}
...
$C\bb{\rho_i(\mb 0),P}$ at rate at least~$e^{-\varepsilon h}$.
...
\end{align*}
The initial law~$\mu$ need not factor over the end-patch sites, so the one-patch
convergence cannot simply be multiplied. Lemma~\ref{lem:end-factor-relaxation}
expands the product in centered moments and gives
\begin{align*}
&\abs{
\CE{W_t^\mu\ind\bb{L_{T,t}}}{\mathcal G_T}
-
...
\\
&\qquad\quad+
e^{-\varepsilon h}B_T\abs{\mathcal E_T}
\prod_{P\in\mathcal E_T}
C\bb{\rho_{i(P)}(\mb 0),P}.
\end{align*}
The first term controls the nonconstant centered terms: each contains at least
one slope which decays by~$e^{-\varepsilon h}$. The second term is the accumulated
error of the constant terms over the end patches.
...
\end{align*}
For the centered-part contribution,~$B_T\leq B_T^\varepsilon$ and the same
conditional formula for the system with pure deaths removed give
\begin{equation*}
\E_A\Cb{
 \ind\bb{E_T^R}B_T
 \CP{L_{T,t}}{\mathcal G_T}
...
\leq1.
\end{equation*}
For the constant-part contribution,~$\abs{\mathcal E_T}\leq\abs R$ on
$E_T^R$, and the second conditional formula above gives
\begin{equation*}
\E_A\Cb{
 \ind\bb{E_T^R}B_T\abs{\mathcal E_T}
 \prod_{P\in\mathcal E_T}
...
\eqref{eq:limiting-death-removed-mass}. Since~$h=t-T$, the two contributions
give~\eqref{eq:no-late-relaxation-bound}.
\end{proof}

% Remaining proof block:
%   spatial confinement, citing the confined-interaction identity.
