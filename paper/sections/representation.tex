...
\begin{proof}
Fix a skeleton realization~$g$ and use the standard Poisson disintegration
convention~\cite{LastPenrose2017}: prescribed atoms are inserted and the
remaining process is Poisson. Conditional on the source, time, and target, a
split or birth has probability proportional to its rate; this conditional law
is well defined even though a fixed list of continuous times has probability
zero.

The patch interiors are disjoint source--time strips. The split--birth kind of
a boundary interaction belongs only to the patch beginning at its source; the
patches beginning at its targets do not use that mark. Hence, before imposing
consistency, the variables~$(\Sigma_P)_{P\in\mathcal P_T(g)}$ are independent
with laws~$(\pP_P)_P$. The skeleton equals~$g$ when
$\operatorname{Con}(P)$ occurs for every patch. Conditioning this product law
on the product event~$\bigcap_P\operatorname{Con}(P)$ gives
\eqref{eq:patch-factorization}. Each normalizer is positive for an occurring
patch: if it vanished, the patch would have zero conditional probability even
after its initial boundary interaction was inserted.
\end{proof}
