...
If~$\mathsf X(P)=\mathsf I$, the initial interaction of~$P$ is incoming from source different than~$i$, and we set~$\Sigma_P=\Sigma_P^\circ$. Let~$\pP_P$ be the
corresponding product Poisson law of~$\Sigma_P$. If~$\mathsf X(P)=\mathsf O$, the initial
...
\eqref{eq:patch-initial-kind-law} and the interior Poisson law of~$\Sigma_P^{\circ}$.
Write~$\E_P$ for expectation under~$\pP_P$.

For a realization of the graphical construction, the local active indicator is
\begin{equation*}
    X_u^P=\ind\cb{i(P)\in A_u} \quad\text{for}\quad s(P)\leq u<e(P). 
\end{equation*}
Under the reference patch
...
when it produces no successful interaction inside the patch and, if the
patch ends with an outgoing interaction, makes that interaction successful by surviving till the patch end.
Thus
\begin{equation*}
\operatorname{Con}(P)
=
...
\end{equation*}
where~$\Omega_P$ is the sample space. The patches for which the consistency event has probability zero almost surely do not occur, so~$\pP_P\bb{\operatorname{Con}(P)}>0$ for all~$P\in\mathcal{P}$ almost surely. The consistent patch law is
\begin{equation*}
