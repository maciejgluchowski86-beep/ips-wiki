...
For \(A\Subset\Lambda\), the action of \(\Lcal\) on \(\chi_A\) is a signed
linear combination of monomials. We write the signed coefficients as rates and
sign labels of a process on finite subsets of \(\Lambda\). The difference
between the resulting jump generator and \(\Lcal\chi_A\) is diagonal and is
recorded by a Feynman--Kac potential.
...
\(B_{i,S}\) with \(S\neq\varnothing\) is a \emph{birth}. Ignoring the sign
coordinate, these updates preserve unions, which accounts for the term
\emph{additive}.
...
\end{equation}
In the expansion of \(\Lcal\chi_A\), the coefficient \(a_i^\delta(S)\)
multiplies the monomial indexed by \((A\setminus\{i\})\cup S\), while
\(a_i^\beta(S)\) multiplies the monomial indexed by \(A\cup S\); see
Appendix~\ref{app:generator-action}.

For every \(S\subseteq N(i)\), define
\begin{equation}
...
\end{equation}
and set \(\beta_i(\varnothing)=0\) and
\(\sigma_i^\beta(\varnothing)=+\).

The coefficient \(a_i^\beta(\varnothing)\) corresponds to an empty-target
birth, which leaves the active set unchanged. It is included in the potential
rather than represented as a jump. Define
\begin{equation}
...
Independent transitions \(1\to0\) act diagonally on monomials. For a
nonnegative profile \(\mathbf d=(d_i)_{i\in\Lambda}\), let
\begin{equation}
...
\(Y_0=(A_0,\sigma_0)\). Suppose the process has been constructed up to time
\(s\) and has active set \(A_s\). Among the Poisson clocks whose sources lie in
\(A_s\), take the first ring after time \(s\). At a mark
\((i,t,\delta,S)\), apply \(D_{i,S}\); at a mark \((i,t,\beta,S)\), apply
\(B_{i,S}\). Repeat from the new state, and keep the process constant between
