...
transitions~$1\to0$ act diagonally on monomials. For a nonnegative profile
$\mathbf d=(d_i)_{i\in\Lambda}$,
\begin{align*}
...
\end{align*}
Adding~$\Ncal^{0,\mathbf d}$ leaves the dual jumps unchanged and replaces
$V(A)$ by~$V(A)-\sum_{i\in A}d_i$. We therefore call these independent
...
$Y_0=(A_0,\sigma_0)$. Suppose the process has been constructed up to time~$s$
and has active set~$A_s$. Among the Poisson clocks whose sources lie in~$A_s$,
take the first ring after time~$s$. At a mark~$(i,t,\delta,S)$, apply
$D_{i,S}$; at a mark~$(i,t,\beta,S)$, apply~$B_{i,S}$. Repeat from the new
state, and keep the process constant between these relevant interactions.
Rings at inactive sources are ignored.

The total rate of relevant clocks is bounded by a constant times~$\abs{A_s}$,
and one interaction creates at most~$\sup_i\abs{N(i)}$ new active sites. Thus
$\abs{A_t}$ is dominated by a nonexplosive continuous-time branching process.
In particular, only finitely many relevant interactions occur on every bounded
time interval. We use the full interaction set~$I$ throughout the patch
construction.
