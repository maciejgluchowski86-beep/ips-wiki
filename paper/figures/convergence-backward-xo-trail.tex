% Schematic for the backward chain in Lemma 6.3.
% Each filled rectangle is an XO patch.  It ends at the source of an
% outgoing successful interaction, and one target starts the next rectangle.
\begingroup
\definecolor{convtrailcone}{gray}{0.92}
\definecolor{convtrailconeborder}{gray}{0.60}
\definecolor{convtrailpatch}{gray}{0.46}
\definecolor{convtrailpatchborder}{gray}{0.34}
\definecolor{convtrailink}{gray}{0.08}
\begin{tikzpicture}[x=0.48cm,y=0.48cm]
\path[use as bounding box] (0.20,-0.12) rectangle (13.64,8.02);
\tikzset{
  convtrail cone/.style={fill=convtrailcone,draw=convtrailconeborder,
    line width=0.52pt,line join=round,rounded corners=2.2pt},
  convtrail patch/.style={fill=convtrailpatch,draw=convtrailpatchborder,
    line width=0.28pt,line join=round},
  convtrail arrow/.style={draw=convtrailink,line width=0.42pt,
    -{Stealth[length=0.72mm,width=0.52mm]}},
  convtrail source/.style={fill=convtrailink,draw=none,circle,
    inner sep=0pt,minimum size=2.15pt},
  convtrail axis/.style={draw=convtrailink,line width=0.58pt,
    -{Stealth[length=1.65mm,width=1.08mm]}},
  convtrail tick/.style={draw=convtrailink,line width=0.52pt},
  convtrail label/.style={font=\scriptsize,text=convtrailink,inner sep=1pt}
}

% A broad cone with deliberately uneven, asymmetric lateral growth.
\path[convtrail cone]
  (5.43,0)
  -- (5.48,0.38)
  -- (5.18,0.64)
  -- (5.25,1.12)
  -- (4.71,1.34)
  -- (4.75,1.93)
  -- (4.00,2.08)
  -- (4.12,2.49)
  -- (3.52,2.86)
  -- (3.59,3.48)
  -- (2.73,3.64)
  -- (2.45,4.18)
  -- (2.56,4.63)
  -- (1.75,4.94)
  -- (1.92,5.58)
  -- (1.17,5.86)
  -- (1.30,6.28)
  -- (0.65,6.64)
  -- (0.78,7.05)
  -- (0.52,7.46)
  -- (12.18,7.46)
  -- (11.92,7.10)
  -- (11.30,6.87)
  -- (11.42,6.48)
  -- (10.72,6.11)
  -- (10.86,5.63)
  -- (10.15,5.39)
  -- (10.22,4.94)
  -- (9.66,4.54)
  -- (9.78,4.05)
  -- (8.90,3.82)
  -- (9.07,3.20)
  -- (8.42,2.92)
  -- (8.52,2.42)
  -- (7.90,2.08)
  -- (8.05,1.50)
  -- (7.52,1.22)
  -- (7.69,0.77)
  -- (7.47,0)
  -- cycle;

% Twenty short patches whose lifetimes partition [0,u).  Consecutive patches
% begin at neighboring targets of the intervening successful interactions.
\foreach \x/\ya/\yb in {
  6.15/0.00/0.34,
  6.57/0.34/0.71,
  6.15/0.71/1.01,
  5.73/1.01/1.43,
  6.15/1.43/1.77,
  5.73/1.77/2.16,
  5.31/2.16/2.48,
  5.73/2.48/2.91,
  6.15/2.91/3.25,
  6.57/3.25/3.62,
  6.15/3.62/3.94,
  6.57/3.94/4.35,
  6.99/4.35/4.68,
  6.57/4.68/5.05,
  6.15/5.05/5.43,
  5.73/5.43/5.75,
  6.15/5.75/6.18,
  6.57/6.18/6.50,
  6.99/6.50/6.91,
  6.57/6.91/7.46
}
{
  \path[convtrail patch]
    ({\x-0.14},\ya) rectangle ({\x+0.14},\yb);
}

% Every patch terminates at the root of a small two-target interaction.
% One arrowhead coincides with the lower endpoint of the next patch.
\foreach \x/\y in {
  6.15/0.34,6.57/0.71,6.15/1.01,5.73/1.43,6.15/1.77,
  5.73/2.16,5.31/2.48,5.73/2.91,6.15/3.25,6.57/3.62,
  6.15/3.94,6.57/4.35,6.99/4.68,6.57/5.05,6.15/5.43,
  5.73/5.75,6.15/6.18,6.57/6.50,6.99/6.91,6.57/7.46
}
{
  \draw[convtrail arrow] (\x,\y)--({\x-0.42},\y);
  \draw[convtrail arrow] (\x,\y)--({\x+0.42},\y);
  \node[convtrail source] at (\x,\y) {};
}

% Minimal time axis; u is the only label in the panel.
\draw[convtrail axis] (13.02,0)--(13.02,7.92);
\draw[convtrail tick] (12.90,7.46)--(13.14,7.46);
\node[convtrail label,anchor=west] at (13.23,7.46) {$u$};
\end{tikzpicture}
\endgroup
