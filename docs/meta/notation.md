...
| $\mathcal L$ | generator | Markov processes |
...
| $\mathbf p^\star=(p_i^\star)_{i\in\Lambda}$ | patch threshold profile | patch positivity |
| $p_i^\circ$ | empty-neighbour equilibrium one-density $c_i^0(\varnothing)/(c_i^0(\varnothing)+c_i^1(\varnothing))$ | patch convergence |
| $\chi_A^*$ | centered monomial $\prod_{i\in A}(\eta(i)-p_i^\star)$ | centered-moment order |
| $\mu\preceq_*\nu$ | centered-moment order: $\mu(\chi_A^*)\le\nu(\chi_A^*)$ for every finite $A$ | patch positivity |
| $\mathcal M_*$ | measures with nonnegative $\chi_A^*$-moments | patch positivity |
| $\mathcal M_{-,K}$ | measures with $(\mu+K\mu_{\mathbf1})/(1+K)\in\mathcal M_*$ | patch convergence |
| $\mathcal M_-$ | $\bigcup_{K\ge0}\mathcal M_{-,K}$ | patch convergence |
...
| $\bar\chi_A$ | vacancy monomial $\prod_{i\in A}(1-\eta(i))$ | algebra/duality |
| $(A_t,\sigma_t)$ | finite-set dual process with $\sigma_t\in\{+,-\}$ | duality |
| $\mathcal I_T,\mathcal I$ | successful-interaction skeleton through $T$, and over all time | signed additive set processes |
| $\mathcal G_T$ | sigma algebra generated by $Y_0$ and $\mathcal I_T$ | patches |
| $\mathcal P_T,\mathcal P$ | horizon-$T$ patch family and all-time patch family | patches |
| $\mathcal B_t,\mathcal E_t$ | bulk and end patch families | patches |
| $P^{\downarrow t}$ | patch $P$ truncated at time $t$ | patches |
| $P^{\uparrow u}$ | geometric extension of an end patch through time $u$ | patches |
| $C(P),C(z,P)$ | averaged full-patch and end-patch contributions | patches |
| $W_t^\mu,W$ | finite-horizon and all-time nonnegative patch weights in the convergence proof | patch convergence |
| $\mathbf{Cone}_T$ | sites reached by successful interactions by time $T$ | patches |
| $E_T^R$ | event $\{\mathbf{Cone}_T\subseteq R\}$ | patch convergence |
| $L_{T,t}$ | no successful interaction in $(T,t]$ | patch convergence |
| $L_T$ | no successful interaction after $T$ | patch convergence |
| $P_t^{R,0}$ | zero-boundary spin-system semigroup on $R$ | spin systems |
| $\mathbf1_X$ | indicator of event or statement $X$ | probability |
| $\pi$ | distinguished invariant measure in the common-limit theorem | ergodicity |
...
$$
\mathcal L f(\eta)=\sum_{i\in\Lambda} c_i(\eta)\bigl(f(\eta^i)-f(\eta)\bigr).
$$
