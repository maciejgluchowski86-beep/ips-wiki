...
If \(c_i^0(S)\le0\), then \(D_{i,S}'\ge D_{i,S}\ge0\). If \(c_i^0(S)>0\), patch positivity gives \(c_i^1(S)\le-c_i^0(S)<0\), and therefore \(D_{i,S}'\ge0\) directly. This proves the nondegenerate patch-positivity criterion whenever the reduced empty-neighbour sum is positive.

If the reduced empty-neighbour sum vanishes, then \(c_i^0(\vn)=0\) and \(c_i^1(\vn)=a_i\). When \(a_i>0\), the original determinant condition gives \(c_i^0(S)\ge0\), so \(c_i^0(S)+c_i^1(S)\le0\) implies \(c_i^1(S)\le0\); when \(a_i=0\), the two systems coincide. Thus the degenerate patch-positivity criterion is also preserved.

Hence \(\cL'\) has patch positivity. Its critical density is unchanged because the critical-density formula depends only on the nonempty-neighbour coefficients.
...
The [general-law end-factor expansion](patch-representation-of-spin-systems.md#averaging-over-a-general-initial-law) gives, for every end-patch family \(\mathcal E\),
