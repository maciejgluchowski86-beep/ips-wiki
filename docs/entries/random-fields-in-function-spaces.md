...
Branching recursions are naturally conditioned on ancestral data. Let \(\mathcal G\) be the sigma-field generated by the current branch time, position, type, and any skeleton information already exposed. A conditional function-space estimate has the form
...
Such estimates are stronger than an unconditional bound after averaging over \(\mathcal G\), but they compose well with a branching argument. If descendant fields \(Y_1,\ldots,Y_m\) are conditionally independent given \(\mathcal G\), then products of suitable functions of the descendants factor conditionally. For example, when the moments exist,

$$
\mathbb E\left[
\prod_{i=1}^m\lVert Y_i\rVert_X^p
\,\middle|\,
\mathcal G
\right]
=
...
This is the function-space analogue of the conditional product factorization used in ordinary branching representations. For the random-patch problem, a recursive regularity theorem will likely need estimates of the form (6) rather than only pointwise or unconditional moments.
