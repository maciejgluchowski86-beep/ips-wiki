...
Once the uniform [logarithmic Sobolev inequality](log-sobolev-modified-log-sobolev.md) is available, the usual semigroup consequences give hypercontractivity and, through the implication LSI $\Rightarrow$ [Poincare](poincare-spectral-gap.md), uniform exponential $L^2$ relaxation. Stroock--Zegarlinski go further: in their finite-range setting the spatial mixing condition and the uniform dynamical relaxation property are equivalent, making this a genuine static-to-dynamic criterion rather than a one-way perturbative estimate.
...
A companion paper by the same authors treats finite spin spaces explicitly. This route is conceptually distinct from the elementary single-site [Dobrushin contraction criterion](dobrushin-influence-contraction.md). Dobrushin--Shlosman conditions are block/spatial mixing conditions and can hold in parameter regimes where a simple one-site influence matrix is too crude.
