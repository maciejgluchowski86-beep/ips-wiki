...
Then $\mathcal M$ contains an invariant probability measure. This is the continuous-time Krylov--Bogoliubov existence theorem.

## Proof

Fix $\mu\in\mathcal M$. For $T>0$, define the Cesaro mean
...
More restrictive preserved classes produce invariant measures with additional properties. For a patch-positive spin system, the [centered-moment cone](high-density-measure.md) $\mathcal M_*$ is nonempty, convex, weakly closed, and preserved by the semigroup, so it contains an invariant probability measure. Under a uniform pure-death component, the stronger [common invariant-limit theorem](common-invariant-limit-under-uniform-pure-deaths.md) identifies a common limit for the larger affine class $\mathcal M_-$.

## Relation to ergodicity

Existence of an invariant measure is weaker than uniqueness or [ergodicity](ergodicity.md). The Cesaro construction produces at least one invariant measure but generally gives neither uniqueness nor convergence of $\mu P_t$ without averaging.
