...
This viewpoint is especially useful when the reference chain has a known [spectral gap](poincare-spectral-gap.md) but makes nonlocal moves, whereas the chain of interest has local or constrained moves. The same architecture can compare generators directly: if one proves \(\widetilde{\mathcal E}\le A\mathcal E\) by any argument, canonical paths are no longer needed explicitly.
...
The exclusion-specific [moving-particle lemma](moving-particle-long-jump-exclusion.md) is a related but sharper comparison mechanism: it replaces a chosen path by an effective-resistance bound exploiting exclusion/interchange algebra.
