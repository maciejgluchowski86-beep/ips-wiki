...
The method is also distinct from [clan-of-ancestors perfect simulation](clan-of-ancestors-perfect-simulation.md): CFTP detects coalescence of a grand random map over initial states, whereas ancestor-clan methods prove that a finite backward dependency graph suffices to reconstruct the target without such a grand coupling.
