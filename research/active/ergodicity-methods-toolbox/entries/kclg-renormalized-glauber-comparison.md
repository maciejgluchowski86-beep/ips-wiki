---
method_id: kclg-renormalized-glauber-comparison
...
targets:
  ...
  - convergence
  - mixing
model_scope: Cooperative kinetically constrained lattice gases, especially the two-dimensional Kob--Andersen model with boundary reservoirs
source_status: primary-checked
primary_source: Nicoletta Cancrini, Fabio Martinelli, Cyril Roberto, Cristina Toninelli, Kinetically Constrained Lattice Gases, Communications in Mathematical Physics 297 (2010), 299-344.
primary_pinpoint: Theorems 4.1-4.2; Section 5 and Theorems 5.5-5.6; Sections 6-8 for the comparison, AGL gap, and polynomial-decay arguments
primary_url: https://doi.org/10.1007/s00220-010-1038-3
application_source: Nicoletta Cancrini, Fabio Martinelli, Cyril Roberto, Cristina Toninelli, Kinetically Constrained Lattice Gases, Communications in Mathematical Physics 297 (2010), 299-344.
application_pinpoint: Theorem 4.1 for finite-box Kob--Andersen relaxation and Theorem 4.2 for infinite-volume variance decay
...
The method is not merely the generic canonical-path comparison already represented elsewhere in the toolbox: the decisive step is first to **manufacture an auxiliary high-density good-block process** for which a useful Poincare inequality exists. Without that renormalized state space, direct paths in the original vacancy field are exponentially unlikely.

The AGL gap theorem itself uses a constrained bisection argument, so there is overlap with the block-dynamics entry, but here bisection is only one component of a larger KCLG-specific bridge from nonconservative long-range refreshes back to cooperative conservative motion.

This entry substitutes for the assigned nonreversible-sector/hypocoercive slot. A clean primary IPS application in which a sector or symmetric-part estimate directly proves quantitative relaxation was not located in this wave; sector-condition sources found during reconnaissance primarily use it for fluctuation/CLT or hydrodynamic estimates rather than convergence to equilibrium.
