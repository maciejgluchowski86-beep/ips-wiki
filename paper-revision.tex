...
Write~$A\Subset\Lambda$ when~$A$ is finite. For~$A\Subset\Lambda$, define
\begin{equation*}
\chi_A(\eta)
:=
\prod_{i\in A}\eta(i),
...
\chi_\vn:=1.
\end{equation*}
If~$F\Subset\Lambda$, then~$\cb{\chi_A:A\subseteq F}$ is a basis for the
functions depending only on the spins in~$F$. For a profile
$\mb p=(p_i)_{i\in\Lambda}\in[0,1]^\Lambda$, let~$\mu_{\mb p}$ denote the
Bernoulli product measure satisfying
\begin{equation*}
\mu_{\mb p}\bb{\eta(i)=1}=p_i
\qquad
\text{for every }i\in\Lambda.
\end{equation*}
For a constant profile, write~$\mu_p$.

Every function~$g$ depending only on the spins in~$N(i)$ has a unique
expansion
\begin{equation}
