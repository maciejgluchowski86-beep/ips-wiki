...
We prove Theorem~\ref{thm:common-invariant-limit} by comparing the finite-horizon
patch representation with its limit on the event that there are finitely many patches, corresponding to the event that the underlying dual process died out. Fixing a cut time~$T<t$, there are three sources of error. First,
a successful interaction may occur after~$T$; the pure-death comparison from
Section~\ref{subsec:proof-pure-death-comparison} makes the contribution of this
event of order~$e^{-\varepsilon T}$. Second, if there is no successful
interaction after~$T$, the end patches still depend on the initial
configuration, but this dependence relaxes at rate~$e^{-\varepsilon(t-T)}$.
Finally, the number of end patches is controlled by confining the
successful-interaction skeleton to a finite set~$R$, while the contribution
from leaving~$R$ is bounded by finite propagation. Taking~$R$ to be a ball of
radius proportional to~$T$ gives~$\abs R\lesssim(1+T)^D$, and we then take
~$T=t/2$.
