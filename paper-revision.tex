...
~$\sigma_i^\beta(S)e^{V_i\Delta}$. These are the remaining two formulas of
~\eqref{eq:full-contribution-formulas}. The formulas extend to
~$\Delta=\infty$ by taking limits.}

\subsection{\textcolor{red}{End-patch relaxation and continuation}}
\label{app:positivity-algebra}

\textcolor{red}{The following identities are used in the convergence proof.
There one fixes the patch configuration up to a cutoff time and, on the event
that no later successful interaction occurs, extends each end patch to a later
time. We need to understand both how its terminal contribution changes under
this extension and what happens as the extension becomes long.}
...
\textcolor{red}{Thus, when~$\lambda_i(\mb 0)>0$, the terminal value~$z$
is forgotten exponentially fast and is replaced in the limit by
~$\rho_i(\mb 0)$. The second identity describes the same relaxation over
two consecutive time intervals.}

Let~$P$ be an end patch and let~$u\geq e(P)$ be finite. The extension
~$P^{\uparrow u}$ is the geometric patch from
\eqref{eq:geometric-patch-extension}. \textcolor{red}{If no successful
interaction occurs before time~$u$, this is the patch obtained by extending
~$P$ to~$u$. Its conditional patch contribution is obtained from
\eqref{eq:end-contribution-formulas} by replacing its length
~$e(P)-s(P)$ by~$u-s(P)$.} In particular,
\begin{equation}
...
\textcolor{red}{There is one distinction needed later. The quantity
~$C(z,P^{\uparrow u})$ is conditioned on the extended patch being consistent.
In the convergence proof we condition only on the patch configuration up to
time~$e(P)$ and then ask that no successful interaction occur during
~$[e(P),u)$. The relevant quantity is therefore the contribution in
\eqref{eq:end-patch-continuation} multiplied by the probability of that event.
The corresponding identity is derived later together with the convergence
estimates.}
