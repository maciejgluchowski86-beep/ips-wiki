...
For~$A\Subset\Lambda$ and~$\sigma\in\cb{+,-}$, identify~$+$ and~$-$ with
$+1$ and~$-1$ and define
\begin{equation}
...
function~\eqref{eq:monomial-duality-function} is fixed, the possible jumps,
their signs, and the potential are read directly from the action of~$\Lcal$
on monomials.
