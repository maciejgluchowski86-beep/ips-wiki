...
Theorem~\ref{thm:common-invariant-limit}. The convergence theorem does not
assume reversibility and does not require the invariant measure to be known in
advance.

There are two routes from this common-limit result to convergence uniformly
...
Corollary~\ref{cor:oriented-ergodicity}. The East model gives the main example
of the second route. Its neighbourhood graph is orientable, so the perturbed
East systems of Corollary~\ref{cor:perturbed-east} are uniformly ergodic for
every~$p\in(0,1)$, including the regime~$p>1/2$.\section{Setup and examples}
\label{sec:setup}
