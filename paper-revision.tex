...
\ABSTRACT{%
In kinetically constrained spin models such as FA-1f and East, adding vacancies
can make more updates legal, but this does not in general make the dynamics
monotone in the usual configuration order. We introduce patch-positive spin
systems, a class characterized by local sign conditions on the update rule.
Each patch-positive system has a threshold profile, and we prove that its
semigroup preserves the corresponding centered monomial order on probability
laws. This gives comparisons between initial laws and between different
dynamics, including systems that differ by additional independent
...
common invariant measure. If the threshold profile is at most~$1/2$, the
system is uniformly exponentially ergodic. If the neighbourhood graph is
orientable, uniform ergodicity holds without this restriction. In particular,
the perturbed East models considered here are uniformly ergodic for every
refresh parameter~$p\in(0,1)$. The centered monomial comparison also gives
new convergence results for the biased annihilating branching process and a
new sufficient ergodicity criterion for one-dimensional simple spin systems
under positive rates.

The proof starts from a standard signed set-valued Feynman--Kac dual. By
conditioning only on its inter-site interactions, we divide each site line
into patches and average the remaining local randomness patch by patch. This
exposes cancellations in the signed dual and leads to the positivity condition
underlying the comparison principle.}
