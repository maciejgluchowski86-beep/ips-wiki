...
{\color{red}
Conditioning on~$\mathcal G_T$ fixes the successful-interaction skeleton and
hence the patch family~$\mathcal P_T$.
}
It leaves unresolved the local information that was omitted from the skeleton.
In particular, deaths inside patches are not recorded, and for a recorded
outgoing interaction the skeleton does not say whether it was a split or a
...
\Sigma_P^\circ
=
\cb{(i,u,\gamma,R)\in {\color{red}I^{\mathrm P}}:s<u<e}.
\end{equation*}
...
\end{equation}
{\color{red}
For an outgoing patch boundary,~$\delta_i(S)+\beta_i(S)>0$ because that
boundary comes from a successful interaction.
}
In this case,
\begin{equation*}
\Sigma_P
...
fixed by the skeleton. {\color{red}A mark with nonempty target inside the patch
would be a successful interaction whenever the patch site is active immediately
before it, and would therefore give an additional successful-interaction record,
contrary to the fixed skeleton.} Hence such marks can only occur with
