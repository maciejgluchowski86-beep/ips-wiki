...
The main construction starts from a signed Feynman--Kac representation for finite spin
monomials. Tracing such an observable backward in time gives a set-valued
process recording the sites on which it depends, while the coefficients of the
spin-system generator produce signs in the representation. Treating these
signed terms separately would discard the cancellations that are important
here. Instead, we condition only on the interactions that transmit dependence
between sites. Along each site line, the retained interaction times divide
spacetime into consecutive regions, which we call \emph{patches}. Once this
interaction structure is fixed, the remaining local randomness separates
between patches and can be averaged within each one. This produces the patch
factorization developed in Sections~\ref{sec:dual} and~\ref{sec:patches}.
...
averaged over patches.\section{Setup and examples}
\label{sec:setup}
