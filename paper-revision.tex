...
To obtain the patch representation, we condition on a~$\sigma$-algebra that
records how dependence passes between sites while revealing as little of the
remaining randomness as possible. We therefore retain every interaction that
actually connects a source site to a nonempty target, together with its source,
time, and target, but suppress local events and any additional information that
does not change which sites are connected. To make this conditioning precise,
we first realize the signed dual graphically.

We use the standard Poisson graphical construction for the signed dual. For
...
$I_{i,\varnothing}^\beta=\varnothing$. We write the resulting collection of
marks as
\begin{equation*}
I^{\mathrm P}
=
...
Each record~$(j,t,S)\in\mathcal I$ is outgoing at its source~$j$ and incoming
...
interaction and the next. In Section~\ref{subsec:patches}, we use these
intervals as the patches over which the unrevealed local randomness is
organized.
...
~$0\in\mathcal T^{\mathsf I}(i)$ for every~$i\in A$, so the initial active
sites enter the same geometry as sites reached by later successful
interactions.
}
...
patches crossing~$t$, truncated at the observation time.
{\color{red}
By Lemma~\ref{lem:dual-local-finiteness}, the family~$\mathcal P_t$ is finite
almost surely.
}
Distinct end patches have distinct sites, since the successful-interaction
times divide each site line into consecutive intervals.
...
{\color{red}times when a site is dual-active}, black arrows are successful interactions,
