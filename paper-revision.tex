...
\chi_\vn:=1.
\end{equation*}
If~$F\subseteq\Lambda$, then~$\cb{\chi_A:A\Subset F}$ is a basis for the local
functions depending only on coordinates in~$F$. For a profile
$\mb p=(p_i)_{i\in\Lambda}\in[0,1]^\Lambda$, let~$\mu_{\mb p} = \otimes_{i\in\Lambda}\operatorname{Ber}(p)$.
