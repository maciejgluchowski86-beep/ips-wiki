...
\textcolor{red}{Fix a patch based at~$i$, and write}
\begin{equation*}
\alpha_i
=
...
\end{equation*}
\textcolor{red}{Thus~$\alpha_i$ is the total rate of outgoing interactions at~$i$, and}
...
\subsection{\textcolor{red}{Bulk and infinite patch contributions}}
...
\Delta=e(P)-s(P).
\end{equation*}
\textcolor{red}{For finite and infinite patches, the contribution is}
\begin{equation}
...
corresponding row is read at~$\Delta=\infty$. \textcolor{red}{If a denominator in~\eqref{eq:end-contribution-formulas} or
\eqref{eq:full-contribution-formulas} vanishes, that patch has zero
probability of occurring, so the formula is irrelevant.}

\textcolor{red}{By the definitions in Section~\ref{sec:dual}, these
quantities satisfy}
\begin{equation}
...
\textcolor{red}{Suppose first that the initial interaction is incoming, so~$X_s^P=1$, and the terminal interaction is incoming or absent,
corresponding to~$\mathsf I$ or~$\mathsf E$. Consistency occurs either when
there is no outgoing interaction before the endpoint, or when the first
outgoing interaction is a death with~$S=\varnothing$.} Therefore
\begin{equation*}
\pP_P\bb{\operatorname{Con}(P)}
=
e^{-\alpha_i\Delta}
...
\varphi_i(\Delta).
\end{equation*}
\textcolor{red}{To compute~$C(z,P)$, we also need the numerator of
the conditional expectation defining the patch contribution,
\begin{equation*}
\E_P\Cb{F(z,P)\ind_{\operatorname{Con}(P)}}.
\end{equation*}
If the first outgoing interaction is a death at time~$u$, the site is active
until~$u$ and contributes~$e^{V_i u}$; if there is no outgoing interaction,
...
\textcolor{red}{Suppose next that the initial interaction is outgoing and the
terminal interaction is incoming or absent, corresponding to~$\mathsf I$ or
~$\mathsf E$.} If~$\alpha(P)=\delta$, the source leaves the dual-active set at
...
\textcolor{red}{the consistency probability and the corresponding
unnormalized contribution are, respectively,}
\begin{equation*}
\frac{
\delta_i(S)+\beta_i(S)\varphi_i(\Delta)
}{
...
\eqref{eq:full-contribution-formulas}. \textcolor{red}{If the terminal interaction is outgoing, the source must
remain dual-active throughout the patch. If the initial interaction is
incoming, the patch contributes~$e^{V_i\Delta}$. If the initial interaction
is outgoing, consistency also forces~$\alpha(P)=\beta$, leaving
~$\sigma_i^\beta(S)e^{V_i\Delta}$. These are the remaining two rows of
~\eqref{eq:full-contribution-formulas}. The formulas extend to
~$\Delta=\infty$ by taking limits.}
...
\eqref{eq:geometric-patch-extension}. \textcolor{red}{Its contribution~$C(z,P^{\uparrow u})$, before multiplying
by the probability of no further successful interaction, is obtained from
~\eqref{eq:end-contribution-formulas} by replacing~$e(P)-s(P)$ with
~$u-s(P)$.} In particular,
\begin{equation}
...
the added interval~$[e(P),u)$. \textcolor{red}{The corresponding identities after including the probability
of no further successful interaction are derived in
