...
We describe the systems considered here in a form modeled on KCSMs. At each
...
formal definition is given in Section~\ref{sec:main-results}.

Patch positivity includes the main examples that motivate the paper but is not
a condition satisfied by all KCSMs. FA-1f and East are patch-positive, whereas
FA-$j$f with~$j\geq2$ need not be. The biased annihilating branching process
...
although in that case the usual configuration monotonicity gives stronger
comparisons. These examples are worked out in
Section~\ref{sec:applications}.

The comparison principle associated with patch positivity is an order on
probability laws rather than on configurations. Each patch-positive system has
a reference profile~$\mb p^\star=(p_i^\star)_{i\in\Lambda}$. For a finite
...
\end{equation*}
We call this the \emph{centered monomial order}.

Our main structural result is that patch-positive dynamics preserve this order.
