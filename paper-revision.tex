...
\subsection{\textcolor{red}{Verification of the duality}}
\label{app:dual-data}

\textcolor{red}{The dual rates, signs, and potential were defined in
Section~\ref{sec:dual}. We verify here that they give the generator identity
\eqref{eq:monomial-generator-duality} and then justify the
infinite-volume Feynman--Kac formula.}

Let~$Y=(A,\sigma)$ and let~$H$ be given
by~\eqref{eq:monomial-duality-function}. \textcolor{red}{By
\eqref{eq:death-split-data}--\eqref{eq:birth-data}, each rate multiplied by
...
\textcolor{red}{It remains to pass from the finite-volume generator identity
to the infinite-volume semigroup.} Fix~$R\Subset\Lambda$ containing~$A$.
The finite-state Feynman--Kac formula, applied to the dual killed at the first
successful interaction leaving~$R$, gives
\begin{equation*}
P_t^{R,0}\chi_A(\eta)
=
\E_A\Cb{
