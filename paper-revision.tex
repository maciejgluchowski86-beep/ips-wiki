...
\begin{equation*}
\bar A=\prod_{k=1}^d[a_k,b_k]\cap\Z^d
\end{equation*}
containing~$A$, and set
\begin{equation*}
U=\cb{x\in\Z^d:x_k>b_k\text{ for some }k\leq d}.
\end{equation*}
...
\begin{proof}
The added refreshes contain independent~$1\to0$ transitions at site~$i$ at
rate~$\varepsilon(1-s_i)$. Thus~$\delta>0$ gives a pure-death component of
rate~$\delta$, and the claim follows from
...
Under the assumptions of
Proposition~\ref{prop:perturbed-fa-babp-convergence}, the following hold.
\begin{enumerate}[label=\textup{(\roman*)}]
