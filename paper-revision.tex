...
We next restrict to realizations with no successful interaction after~$T$. The
...
continuation identities in Appendix~\ref{app:no-late-continuation} give
\begin{equation*}
L_{T,t}
=
...
the probability-weighted end factor in
\eqref{eq:main-local-end-relaxation} therefore relaxes to
...
\prod_{P\in\mathcal B_T}C^\varepsilon(P).
\end{equation*}
The same continuation identities give
\begin{align}
...
~$B_T\leq B_T^\varepsilon$, and the corresponding continuation identity for
the system with pure deaths removed gives
\begin{align*}
&\E_A\Cb{
 \ind\bb{E_T^R}
 B_T
...
It remains to control realizations for which the interaction cone leaves~$R$.
...
Proposition~\ref{prop:confined-interaction-identity} makes precise that fixing
the spins outside~$R$ to~$0$ therefore does not change the contribution of
such a realization during the first~$T$ units of dual time. It gives
\begin{equation}
