...
Suppose that the spin system is patch-positive. If~$\mu\preceq_\star\nu$, then
\begin{equation}
\label{eq:centered-monomial-monotonicity}
\mu P_t\preceq_\star\nu P_t
\qquad
...
The theorem gives canonical monotone evolutions at the bottom of
...
\item The family~$\mu_{\mb p^\star}P_t$ is increasing in~$t$ under
...
the same probability measure~$\pi$. Then~$\mu P_t\Rightarrow\pi$.
If, for some~$\rho(t)\to0$ and every~$A\Subset\Lambda$,
\begin{equation*}
\max_{\lambda\in\cb{\underline\mu,\overline\mu}}
\abs{(\lambda P_t)(\chi_A^\star)-\pi(\chi_A^\star)}
\leq
C_A\rho(t),
\end{equation*}
then for every local function~$f$ there is~$C_f<\infty$ such that
\begin{equation*}
\abs{(\mu P_t)(f)-\pi(f)}
\leq
C_f\rho(t).
