...
\textcolor{red}{This section constructs the signed set-valued dual used in the
patch representation. Set-valued dual constructions and Feynman--Kac duality
for spin systems are standard; the features needed here are the signed
monomial representation, the effect of pure deaths on its Feynman--Kac weight,
and a graphical realization in which every interaction has a source and a
target. Section~\ref{sec:patches} will condition this graphical construction
to obtain the patch representation.}
...
\end{equation*}
\textcolor{red}{We now construct the signed set-valued process appearing in
\eqref{eq:intro-signed-duality}. In the notation of this section, the
representation is}
\begin{equation}
...
We say that the signed dual satisfies \emph{Feynman--Kac integrability} if
\begin{equation}
\tag{FK}
...
\end{equation}
\textcolor{red}{Under Feynman--Kac integrability}, for every~$A\Subset\Lambda$,
...
{\color{red}
The terminology of a pure-death component is particularly convenient in the
dual representation. The pure-death generator acts diagonally on monomials:
\begin{equation*}
\Ncal^{\mb 0}\chi_A=-\abs{A}\chi_A.
\end{equation*}
Consequently, adding~$\varepsilon\Ncal^{\mb 0}$ leaves the signed dual jump
process unchanged and replaces the potential by
\begin{equation*}
V(A)-\varepsilon\abs{A}.
\end{equation*}
Thus pure deaths enter the dual only through an additional factor that
decreases the Feynman--Kac weight.
}

\subsection{Graphical construction}
\label{subsec:dual-graphical-construction}

\textcolor{red}{Section~\ref{sec:patches} will retain only part of the
graphical information. We therefore distinguish explicitly the source, target,
and kind of every Poisson mark, as well as whether its source is active when
the mark occurs.} We use the standard Poisson graphical construction for
set-valued particle systems, with notation adapted to the interactions above.
Thus each possible~$D_{i,S}$ or~$B_{i,S}$ update is assigned an independent
Poisson clock. For every~$i\in\Lambda$ and~$S\subseteq N(i)$, let
...
If the signed dual starts from a finite active set, then almost surely only
finitely many Poisson marks occur at active sources on every bounded time
interval.
\end{lemma}

\begin{proof}
The total rate of marks whose source is active is bounded by~$C\abs{A_t}$ for
some finite~$C$, uniformly in the state. One mark can add at most
\begin{equation*}
\sup_{i\in\Lambda}\abs{N(i)}<\infty
\end{equation*}
new active sites. Hence~$\abs{A_t}$ is dominated by a nonexplosive
continuous-time branching process, which proves the claim.
\end{proof}

The construction above records every Poisson mark. In
Section~\ref{sec:patches} we retain much less information: only
nonempty-target interactions that occur at active sources, and only their
source, time, and target. This retained information determines the patch
decomposition.
}
