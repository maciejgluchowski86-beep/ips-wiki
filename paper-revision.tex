...
patch-positive and its threshold profile is the same as that of~$\Lcal$. Indeed,
\eqref{eq:pure-death-decomposition-revision} changes only the empty-set
M\"obius coefficient of~$\lambda_i$, while~$\lambda_i\rho_i=\lambda_i'\rho_i'$,
and~\eqref{eq:threshold-profile} only involves nonempty coefficients. For an end patch~$P$ based at
site~$i$, set
\begin{equation*}
