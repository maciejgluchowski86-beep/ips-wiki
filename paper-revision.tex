...
\end{equation}
where~$\zeta_I(i) = \ind\bb{p_i^\star > 0}$ for~$i\in I$,~$\nu$ is a probability measure,~$\alpha_I>0$ and~$\xi_1,\dots,\xi_n$ and~$s_1+\dots +s_n\geq 1$ are arbitrary boundary conditions and their durations. This is useful because~$\delta_{\zeta_I}\in\mathcal{M}_\star^I$.
...
&\quad +(1-\alpha_I)^m\nu_t(\chi_A^\star)\\
&\geq-\norm{\chi_A^\star}_\infty(1-\alpha_I)^m,
\end{align*}
...
For every finite~$B\subseteq\Lambda$, the~\eqref{eq:convergence-towards-m-star} implies
\begin{equation*}
\mu(\chi_B^\star)
