...
dual process, successful-interaction skeleton, patch families, and consistent
patch laws. Their one-site potentials satisfy
\begin{equation}
\label{eq:pure-death-potential-comparison}
V_i'=V_i+d_i.
\end{equation}

We next compare the patch contributions. From
\eqref{eq:pure-death-decomposition-revision},
\begin{equation}
\label{eq:pure-death-rate-identities}
\lambda_i=\lambda_i'+d_i,
\qquad
\lambda_i\rho_i=\lambda_i'\rho_i'.
\end{equation}
Consequently, for every nonempty~$S\subseteq N(i)$,
\begin{equation}
\label{eq:pure-death-mobius-identities}
\widehat{\lambda_i}(S)=\widehat{\lambda_i'}(S),
\qquad
\widehat{\lambda_i\rho_i}(S)
=
\widehat{\lambda_i'\rho_i'}(S).
\end{equation}
The data~$\alpha_i$, $\delta_i(S)$, $\beta_i(S)$ and their signs in
Appendix~\ref{app:contribution-formulas} are therefore the same for the two
systems, and so is~$\varphi_i$. Only the all-zero-neighbourhood relaxation
changes. Writing~$\psi_i'$ for the function associated with~$\Lcal'$, we
...
Suppose first that a full patch starts with an incoming boundary. The first two
...
0\leq C(P)\leq C'(P).
\end{equation*}
If a full patch starts with an outgoing boundary with target~$S$, patch
positivity gives
\begin{equation*}
\beta_i(S)\sigma_i^\beta(S)
=
...
0\leq C(P)\leq C'(P)
\qquad
\text{for every full patch }P.
\end{equation}
Moreover, if~$P$ has an outgoing terminal boundary, its contribution is the
common sign, when present, multiplied by~$e^{V_i\Delta}$, where
...
By Proposition~\ref{prop:patch-contribution-positivity},
\eqref{eq:pure-death-bulk-patch-comparison} implies that~$\Lcal'$ is
patch-positive. Its threshold profile is the same as that of~$\Lcal$, because
the formula~\eqref{eq:threshold-profile} only involves the nonempty
coefficients in~\eqref{eq:pure-death-mobius-identities}.

It remains to compare the end-patch factors. For an end patch~$P$ based at
site~$i$, set
\begin{equation*}
