...
The signed duality formula of Section~\ref{sec:dual} expresses~$P_t\chi_A$ as an expectation involving the sign, the Feynman--Kac weight, and the terminal dual set. To make the cancellations in this expectation explicit, we condition on only part of the graphical construction: the successful interactions that connect different sites. For each site, the successive incoming and outgoing successful interactions divide the time axis into intervals, which we call patches. Conditional on the successful-interaction skeleton, the unresolved randomness factorizes over these patches. Taking the corresponding conditional expectations gives one contribution for each patch and leads to the patch representation used in the comparison and convergence proofs.
