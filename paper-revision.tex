...
The main construction reorganizes a signed dual representation of the spin
monomials. For~$\chi_A(\eta)=\prod_{i\in A}\eta(i)$, the starting point is
...
\end{equation}
Here~$A_s$ is the finite set-valued dual,~$\sigma_s$ records its sign, and~$V$
...
signed, so estimating the different dual histories separately would lose
cancellations between them.

The new step is to reveal only part of the graphical construction. We let
$\mathcal G_t$ record the inter-site interactions that actually occur in the
dual up to time~$t$, including their times and the sites they connect, while
forgetting their local type and the purely local events between them. Once this
information is fixed, the recorded interactions divide each site line into
...
product of patch-local factors. Consequently, schematically,
\begin{equation}
...
=
\prod_{\substack{P\text{ ending}\\\text{before }t}} C(P)
\prod_{\substack{P\text{ reaching}\\\text{time }t}}
C\bb{\eta\bb{i(P)},P}.
\end{equation}
The notation in~\eqref{eq:intro-patch-factorization} is only schematic; the
patches and their contributions are defined precisely in
Section~\ref{sec:patches}. Its point is that dual histories which differ only
through hidden events inside one patch are averaged together. Positive and
negative terms can therefore cancel locally inside a patch, before the
contributions from different patches are multiplied.

This representation explains the condition introduced above. The coefficient
inequalities defining patch positivity ensure that the averaged contribution
of every patch completed before the observation time is nonnegative. Patches
that reach the observation time retain factors depending on the terminal
spins, and the threshold profile~$\mb p^\star$ is chosen so that these factors
are compatible with the centered monomials~$\chi_A^\star$. The resulting
factorization preserves the sign needed for the centered monomial comparison,
which yields Theorem~\ref{thm:centered-monomial-monotonicity}. Thus patch
positivity is an algebraic condition on the local update rule whose
probabilistic meaning becomes visible only after the signed representation is
