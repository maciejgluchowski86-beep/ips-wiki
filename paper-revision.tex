...
For a profile~$\mb p\in[0,1]^\Lambda$,
\begin{equation*}
\mu_{\mb p}\bb{\chi_A^\star}
=
\prod_{i\in A}\bb{p_i-p_i^\star}.
\end{equation*}
Hence~$\mu_{\mb p}\in\mathcal M_\star$ if and only if
$\mb p\geq\mb p^\star$. Moreover, if
\begin{equation*}
\mb p^\star\leq\mb p\leq\mb q,
\end{equation*}
then
\begin{equation*}
\mu_{\mb p}\preceq_\star\mu_{\mb q}.
\end{equation*}
\end{example}
...
expectation zero. In general~$\mathcal M_\star$ need not have a maximum. If
\begin{equation*}
\mb p^\star\leq\mbs{\tfrac12},
\end{equation*}
then~$\mu_{\mb 1}$ is a maximum: for every~$A\Subset\Lambda$ and
$\eta\in\Omega$,
\begin{equation*}
\chi_A^\star(\eta)
\leq
