...
$\Delta=e(P)-s(P)\in(0,\infty]$. The contribution formulas in
Appendix~\ref{app:bulk-formulas} show that, when the initial boundary is
incoming,
\begin{equation*}
C(P)
=
...
\end{cases}
\end{equation*}
For every realized patch,~$\varphi_i(\Delta)>0$, while
\begin{equation*}
\delta_i(\varnothing)
=
\widehat{\lambda_i\rho_i}(\varnothing)
...
\geq0
\end{equation*}
implies~$\psi_i(\Delta,1)\geq0$. Hence every full patch with an incoming
initial boundary has nonnegative contribution. Only patches with an outgoing
initial boundary can impose sign restrictions.
