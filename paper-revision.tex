...
probability measure. Since~$\mu_{\mb 1}\in\mathcal M_\star$ and~$\Omega$
is compact, the family~$\mu_{\mb 1}P_t$ has a weakly convergent subsequence.
Let~$\pi$ be one such subsequential limit. Then
\begin{equation}
...
\text{for every }A\Subset\Lambda.
\end{equation}
The monomials determine every local function, so every subsequential limit has
the same finite-dimensional marginals. Hence
\begin{equation*}
\mu_{\mb 1}P_t
\Rightarrow
\pi
...
The measure~$\pi$ is invariant. Indeed, if~$g$ is local and~$s\geq0$, then
the Feller property gives
\begin{equation*}
(\pi P_s)(g)
=
...
\pi(g).
\end{equation*}
Thus~$\pi P_s$ and~$\pi$ agree on every local function and therefore are
equal.
...
\text{for every }A\Subset\Lambda.
\end{equation*}
Thus every probability measure belongs to~$\mathcal M_{-,1}$. Applying
Theorem~\ref{thm:common-invariant-limit} with~$K=1$ gives
\eqref{eq:uniform-pointwise-convergence}, after absorbing the fixed factor~$3$
into~$K_f$.
...
0.
\end{equation*}
Therefore~$\nu=\pi$, so~$\pi$ is the unique invariant measure.
\end{proof}
