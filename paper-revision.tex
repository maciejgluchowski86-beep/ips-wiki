...
$I_{i,\varnothing}^\beta=\varnothing$. The full Poisson interaction record is
\begin{equation*}
I^{\mathrm P}
=
\cb{
...
\end{equation*}
For a mark~$(i,t,\alpha,S)\in I^{\mathrm P}$, we call~$i$ its
\emph{source},~$S$ its \emph{target}, and~$\alpha$ its \emph{kind}. The
record~$I^{\mathrm P}$ contains every Poisson mark, including marks whose
source is not active when they occur.

Fix an initial state~$(A_0,\sigma_0)$. At a mark
~$(i,t,\delta,S)\in I^{\mathrm P}$ with~$i\in A_{t-}$, set
\begin{equation*}
(A_t,\sigma_t)
=
D_{i,S}(A_{t-},\sigma_{t-}),
\end{equation*}
and at a mark~$(i,t,\beta,S)\in I^{\mathrm P}$ with~$i\in A_{t-}$, set
\begin{equation*}
(A_t,\sigma_t)
=
B_{i,S}(A_{t-},\sigma_{t-}).
\end{equation*}
If~$i\notin A_{t-}$, the mark does not change the process. Thus, at an active
source, a~$(\delta,\varnothing)$ mark is a death, a~$(\delta,S)$ mark with
~$S\neq\varnothing$ is a split, and a~$(\beta,S)$ mark is a birth. Between
marks that change the state,~$(A_t,\sigma_t)$ is constant.

For the patch construction, it is convenient to treat the initial active set
as arising from an interaction at time~$0$. We therefore introduce the
...
\end{equation}
where~$\infty\notin\Lambda$ is a formal source. This interaction activates
~$A_0$ with sign~$\sigma_0$. We include it in the interaction record by
setting
\begin{equation*}
I
=
\cb{\iota_0}\cup I^{\mathrm P}.
\end{equation*}
