...
\quad\text{for }B\neq A.
\end{equation}
For each~$A$, only finitely many coefficients~$M(A,B)$ are nonzero, and the matrix~$M$ is Metzler, so for each~$t\geq 0$ the matrix~$e^{tM} = \sum_{n\in\N} \frac{t^n}{n!} M^n$ is well-defined and has non-negative entries. If~$\sigma$ is a
...
Since~$\mu_{\mb p^\star}$ is the minimum of~$\mathcal M_\star$ and
~$P_t$ preserves~$\mathcal M_\star$, for every~$s\geq0$,
\begin{equation*}
\mu_{\mb p^\star}
\preceq_\star
\mu_{\mb p^\star}P_s.
...
the maximum of~$\mathcal M_\star$. Since~$\mu_{\mb1}P_s\in\mathcal
M_\star$,
\begin{equation*}
\mu_{\mb1}P_s
\preceq_\star
\mu_{\mb1}.
\end{equation*}
Applying the monotonicity theorem gives
\begin{equation*}
\mu_{\mb1}P_{t+s}
\preceq_\star
\mu_{\mb1}P_t
...
\text{for every }s,t\geq0.
\end{equation*}
Thus every centered monomial expectation decreases to a limit. The same
argument as above gives~$\mu_{\mb1}P_t\Rightarrow\pi^+$ for some
~$\pi^+\in\mathcal M_\star$, and the semigroup property shows that
~$\pi^+$ is invariant.
