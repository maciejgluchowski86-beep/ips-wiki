...
We use the standard Poisson graphical construction for the jump process defined
above. For every~$i\in\Lambda$ and~$S\subseteq N(i)$, let
...
\emph{target}, and~$\alpha$ its \emph{kind}.

Starting from~$(A_0,\sigma_0)$, a mark~$(i,t,\delta,S)$ with
~$i\in A_{t-}$ applies~$D_{i,S}$, while a mark~$(i,t,\beta,S)$ with
~$i\in A_{t-}$ applies~$B_{i,S}$. Marks whose source is not active do not
change the process. Thus the Poisson construction has generator~$\Dcal$ from
\eqref{eq:signed-dual-generator}.

The interactions through which dependence passes between distinct sites are
exactly the marks with nonempty target that occur at active sources. We call
such a mark~$(i,t,\alpha,S)$ a \emph{successful interaction}. Its source
~$i$, time~$t$, and target~$S$ determine which site lines are connected by the
interaction. The kind~$\alpha$ is not needed for this information: a split and
a birth with the same source and target connect the same sites. A death has
empty target and therefore does not transmit dependence to another site, while
a mark at an inactive source does not affect the dual at all.
