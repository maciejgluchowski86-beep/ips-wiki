...
monotone, traditionally called attractive in the interacting-particle-systems
...
In the standard binary models relevant here, a site may update only when a
...
constraint using only designated forward neighbours. Thus these models have a
...
$\xi$. If the two configurations agree at the updated site and the legal
refresh in~$\eta$ changes that spin from~$0$ to~$1$, the order can be lost.
Thus monotonicity of the constraint does not imply monotonicity of the spin
system.

This raises the question that motivates the paper: how does the monotonicity
of the unconstrained refresh dynamics survive after kinetic constraints are
imposed? More generally, can one recover a useful comparison principle for
