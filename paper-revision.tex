...
laws defined by monomials centered around a density profile inherent to the system. This yields comparison principles between initial laws and between
...
vacancies can make more updates legal, but this does not produce the usual
...
$\xi$. For example, if for two neighboring sites~$i\sim j$ we have~$\eta(x) = \ind(x\neq i,j)$ and~$\xi(x)=\ind(x\neq i)$, then~$\eta\leq \xi$ but and update at~$i$ turns~$\eta(i)\to 1$ and leaves~$\xi(i)=0$, so the order is lost.
...
The duality formula~\eqref{eq:monomial-feynman-kac-duality} represents the nonnegative~$P_t\chi_A$ in a way that includes the sign process~$\sigma_t$. If the sign process was not there then monotonicity in monomial order would be immediate:
\begin{equation*}
 \mu P_t \chi_A \E\Cb{\exp\bb{\int_0^t V(A_s)ds}\mu (\chi_A)}\leq \E\Cb{\exp\bb{\int_0^t V(A_s)ds}\nu (\chi_A)} \leq \nu P_t,
\end{equation*}
for any~$\mu\leq \nu$. Hence, to get monotonicity one needs to control the cancellations from the sign process.
To capture these cancellations, we introduce a~$\sigma$-algebra that reveals when sites interact, but not exactly how. For each site~$i$, the consecutive incoming and outgoing interactions divide~$\cb{i}\times [0,\infty)$ into intervals, which we call patches. Conditional on the the interaction skeleton the law of the dual process factorizes over these patches. Then~$P_t\chi_A$ can be represented via expectation over a product of contributions, one per patch, and given patch positivity these contributions capture all necessary cancellations.
...
\subsection{Preservation of~\texorpdfstring{$\mathcal{M}_*^I$}{M-star-I} under arbitrary boundary conditions}
