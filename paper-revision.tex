...
~$X_s^P=1$, and the terminal label is~$\mathsf I$ or~$\mathsf E$.} Consistency occurs either when there is no
...
\varphi_i(\Delta).
\end{equation*}
\textcolor{red}{The corresponding unnormalized expectation, with terminal
factor~$z$, is}
\begin{equation*}
\delta_i(\varnothing)
...
\textcolor{red}{Suppose next that the initial boundary is outgoing and the
terminal label is~$\mathsf I$ or~$\mathsf E$.} If~$\alpha(P)=\delta$, the source leaves the dual-active set at
...
the source to remain dual-active throughout the patch. \textcolor{red}{If the
initial boundary is incoming, the patch contributes~$e^{V_i\Delta}$. If the
initial boundary is outgoing, consistency also forces~$\alpha(P)=\beta$,
leaving~$\sigma_i^\beta(S)e^{V_i\Delta}$. These are the remaining two rows
of~\eqref{eq:full-contribution-formulas}. The formulas extend to
