...
contact process the centered moment order reduces to the usual configuration order. The final application is partial progress on the 
positive-rates problem for simple spin systems.

\subsection{FA-1f and the East model}
\label{subsec:fa-east-applications}

For FA-1f, the only nonzero nonconstant coefficient of the update rate
\begin{equation*}
\lambda_i=1-\chi_{N(i)}
\end{equation*}
is
...
The East model is the FA-1f dynamics with
\begin{equation*}
\Lambda=\Z^d,
\qquad
N(i)=\cb{i+e_k:k\leq d}.
\end{equation*}
Hence it is patch-positive with threshold~$p^\star=p$. In addition, its
neighbourhood graph is orientable, as observed in Section~\ref{sec:setup}.
This additional geometry will allow us to obtain uniform ergodicity for its
perturbations for every~$p\in(0,1)$, without the restriction~$p\leq1/2$.
