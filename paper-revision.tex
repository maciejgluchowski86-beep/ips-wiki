...
The coefficient conditions in Definition~\ref{eq:patch-positivity} control the
signs that remain after the local averaging in the patch representation. The
explicit contribution formulas are collected in
Appendix~\ref{app:contribution-formulas}. Their consequences needed below are
summarized in the following proposition.

\begin{proposition}[Patch positivity of the contributions]
\label{prop:patch-contribution-positivity}
Suppose that the spin system is patch-positive. Then every full patch satisfies
\begin{equation*}
C(P)\geq0.
\end{equation*}
...
\geq0
\end{equation*}
gives~$\psi_i(\Delta,1)\geq0$. Hence every full patch with an incoming
...
\eqref{eq:outgoing-full-patch-numerator} are nonnegative by patch positivity.
This proves~$C(P)\geq0$ for every full patch.
...
An incoming-initial end patch has, by~\eqref{eq:end-contribution-formulas},
\begin{equation*}
C(p,P)
=
...
\end{equation*}
because~\eqref{eq:all-zero-neighbourhood-relaxation} expresses
$\psi_i(\Delta,p)$ as a convex combination of~$p$ and~$\rho_i(\mb 0)$.
Thus only outgoing-initial end patches impose a restriction. For such a patch
...
$\widehat{\lambda_i\rho_i}(S)\geq0$, so~$S$ imposes no restriction on~$p$.
If~$\widehat{\lambda_i}(S)<0$, nonnegativity for every patch length is
equivalent to
\begin{equation*}
p
\geq
...
$N_{i,S}(0,p)<0$. Since~$\beta_i(S)>0$, an outgoing birth with target~$S$
can start an~$\mathsf{OE}$ end patch, and by continuity its contribution is
negative for all sufficiently small positive patch lengths.

The same formulas also show that every end contribution is nondecreasing in its
terminal argument. For an incoming-initial end patch,
\begin{equation*}
\partial_zC(z,P)
=
...
\geq0,
\end{equation*}
while for an outgoing-initial end patch,
\begin{equation*}
\partial_zC(z,P)
=
\frac{
