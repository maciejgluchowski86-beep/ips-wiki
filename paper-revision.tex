...
h=t-T.
\end{equation*}
The patch~$P^{\uparrow t}$ has length~$\Delta+h$. Therefore
...
\end{equation*}
In both cases this is~$C(\psi_i(h,z),P)$.
...
\begin{equation*}
\psi_i(h,1)\longrightarrow\rho_i(\mb 0)
...
\begin{proof}
Put~$h=t-T$. For an end patch~$P$ based at~$i$, define
\begin{align*}
a_t(P)
&=
\CP{L_{T,t}^P}{\mathcal G_T}
...
a_t(P)
=
C\bb{\psi_i(h,p_i^\star),P},
\qquad
k_t(P)
=
e^{-\lambda_i(\mb 0)h}\kappa(P).
\end{equation}
...
k_t^\varepsilon(P)
=
e^{-\bb{\lambda_i(\mb 0)-\varepsilon}h}
\kappa^\varepsilon(P).
\end{equation*}
