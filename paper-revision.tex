...
with profile~$\mb p^\star$ evolves monotonically in time and converges to an
invariant measure; when~$\mb p^\star\leq\mbs{\tfrac12}$ there is also a
monotone evolution from the all-$1$ configuration. More generally, initial
laws can be compared by placing them between laws whose evolutions are already
understood; see Corollary~\ref{cor:extremal-limits-comparison}. The order also
compares different dynamics. In particular, if a patch-positive system is
obtained from another one by adding independent~$1\to0$ transitions, then,
...
graphs of polynomial growth, if a patch-positive system contains independent
$1\to0$ transitions at a uniformly positive rate, then a broad class of
initial laws converges to a common invariant measure; this class includes every
configuration with only finitely many~$0\section{Setup and examples}
\label{sec:setup}
...
initial configurations and exponentially fast
\cite{GluchowskiMenz2025}. In a different direction, orientability of the
...
model: the perturbations considered in Section~\ref{subsec:perturbed-fa-babp}
are uniformly ergodic for every~$p\in(0,1)$, because the East neighbourhood
graph is orientable.
