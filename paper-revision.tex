...
records exactly how much positivity is needed at the patches that remain open
at the observation time.

The patch contributions can be computed from the local update rule. The
one-patch calculation shows that positivity of all completed patches, together
with the threshold condition for the remaining patches, is equivalent to the
coefficient inequalities used to define patch positivity and~$\mb p^\star$
in Section~\ref{sec:main-results}. The same inequalities make the
off-diagonal coefficients nonnegative when the generator is written in the
centered monomial basis~$\chi_A^\star$. This is what yields preservation of
the centered monomial order in
Theorem~\ref{thm:centered-monomial-monotonicity}. In this way, the algebraic
condition in Section~\ref{sec:main-results} is the local expression of the
cancellations exposed by the patch representation.
