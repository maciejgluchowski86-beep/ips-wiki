...
B(A,n+1)=N_*\bb{B(A,n)}.
\end{equation*}
For~$i\in\Lambda$, write~$B(i,n)=B(\cb{i},n)$. We say that the
...
\begin{example}[One-sided neighbourhoods]
Let~$\Lambda=\Z$ and suppose that
$N(i)\subseteq\cb{j\in\Z:j>i}$ for every~$i\in\Z$. For a nonempty finite
set~$A\subseteq\Z$, take
\begin{equation*}
\bar A=[\min A,\max A]\cap\Z,
\qquad
U=\cb{j\in\Z:j>\max A}.
\end{equation*}
Then both~$U$ and~$\bar A\cup U$ are autonomous, so the neighbourhood graph
is orientable.
\end{example}
