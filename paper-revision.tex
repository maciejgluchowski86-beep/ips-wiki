...
-\widehat{\lambda_i}(S).
\end{equation}
Thus the first family of terms replaces~$i$ by~$S$, while the second adds~$S$
to the active set. In the second sum,~$S=\varnothing$ leaves~$A$ unchanged
and is therefore diagonal; in particular,
\begin{equation*}
a_i^\beta(\varnothing)
=
...
\end{align*}
Thus~$D_{i,\varnothing}$ removes~$i$ from the active set, while
...
reproduced by~$\Dcal H$. The Markov generator~$\Dcal$ also subtracts the
total jump rate from the diagonal, whereas the remaining diagonal term in
~\eqref{eq:signed-dual-generator-expansion} is
~$\sum_{i\in A}a_i^\beta(\varnothing)\chi_A$. The required
Feynman--Kac potential is therefore
\begin{equation}
