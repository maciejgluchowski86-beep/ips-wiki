...
Equivalently,~$p_i^\star$ is the smallest~$p\in[0,1]$ such that
\begin{equation}
\label{eq:threshold-minimality}
...
\end{equation}
Indeed, when~$\widehat{\lambda_i}(S)<0$, this inequality is equivalent to
$p\geq\widehat{\lambda_i\rho_i}(S)/\widehat{\lambda_i}(S)$, while when
$\widehat{\lambda_i}(S)=0$ it follows from patch positivity. In particular,
\eqref{eq:patch-positivity} implies
\begin{equation*}
