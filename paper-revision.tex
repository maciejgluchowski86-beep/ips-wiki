...
contributions nonnegative and therefore allows termwise comparison. A different
...
1-\delta
\end{equation*}
for some~$\delta>0$, then realizations containing many patches have small
total contribution, exponentially in the number of patches. On the other hand,
if there are only few patches over a long time interval, then some of them must
be long, and their terminal contributions depend only weakly on the initial
configuration. This is the usual many-events/few-events mechanism in duality
arguments for ergodicity. Thus the patch representation may remain useful well
outside the patch-positive class, provided the signed patch contributions can
be controlled in absolute value or through cancellations. Models such as
...
feature of the proof rather than a dynamical threshold. The value~$1/2$ enters
...
available, Theorem~\ref{thm:common-invariant-limit} gives convergence to the
common invariant measure. Orientability provides one mechanism for this
burn-in by reducing the problem locally to a finite system with autonomous
boundary. Finding a replacement for this mechanism on general neighbourhood
