...
In this section, we use the standard set-valued Feynman--Kac duality for
spin systems in the signed form needed for the patch representation; see
~\cite{Harris1978,Griffeath1979,Gray1986,JansenKurt2014}. In
Section~\ref{subsec:signed-set-process}, we identify the signed dual directly
from the action of the generator on monomials and record the effect of pure
deaths on its Feynman--Kac potential. In
Section~\ref{subsec:dual-graphical-construction}, we give the Poisson
graphical realization from which the patch construction in
Section~\ref{sec:patches} retains its interaction record. The duality itself
is standard; the new ingredient used later is the conditioning over patches.

\subsection{Signed monomial duality}
...
\end{equation*}
Thus a pure-death component appears in the dual representation only through
an additional damping factor in the Feynman--Kac weight.
