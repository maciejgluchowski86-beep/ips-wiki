...
\frac{p_{1\mid0\eta(i+1)}}{\lambda_i(\eta)}.
\end{equation*}
Exchanging the roles of~$0$ and~$1$ swaps the two values of~$\lambda_i$ and
leaves~\eqref{eq:simple-positive-rates-criterion} unchanged. We may therefore
assume that~$\lambda_i(1)\leq\lambda_i(0)$. Since
$N(i)=\cb{i+1}$,
\begin{align*}
...
same normalization as in~\cite{GluchowskiMenz2025}. Exchanging the two states
and multiplying all transition rates by the same positive constant reduce the
problem to the two faces
\begin{equation*}
p_{1\mid11}=0
\qquad\text{or}\qquad
p_{1\mid10}=0.
\end{equation*}
The positive-rate part of the second face is already known to be ergodic
...
\caption{Sections of the face~$p_{1\mid11}=0$. The horizontal coordinate is
...
by~\cite{GluchowskiMenz2025},
\[
p_{1\mid10}<\tfrac12
\quad\text{or}\quad
p_{1\mid10}<p_{1\mid01}+p_{1\mid00}
\quad\text{or}\quad
p_{1\mid01}\leq p_{1\mid00}.
\]
Dark gray is the additional region from~\cite{GluchowskiMenz2026},
\[
\max\cb{p_{1\mid01},p_{1\mid00}}
<
\sqrt2(1-p_{1\mid10}),
\]
and red is the additional region given by
...
(1-p_{1\mid10})p_{1\mid01}.
\]
White denotes parameters not covered by these criteria. The additional red
region is empty in the~$p_{1\mid00}=1/4$ section. The figure format is adapted
from Figure~3 of~\cite{GluchowskiMenz2026}.}
