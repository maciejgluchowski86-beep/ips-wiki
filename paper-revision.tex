...
Lemma~\ref{lem:no-late-relaxation}. Fix~$0\leq T<t$ and condition on~$\mathcal G_T$. If no successful interaction occurs after time~$T$, the bulk
patches are unchanged and each end patch at time~$T$ is extended forward in
time.

For~$P\in\mathcal E_T$, define its continuation after~$T$ using the future
interactions at~$i(P)$, with no incoming interaction from another site. Let
~$L_{T,t}^P$ be the event that this continuation has no successful interaction
in~$(T,t]$. Distinct end patches are based at distinct sites, so conditional on
...
\CP{L_{T,t}^P}{\mathcal G_T}.
\end{equation}
Indeed, before the first successful interaction after~$T$, no incoming
interaction has changed the activity of any site. Its source must therefore
belong to one of the end patches present at time~$T$, and its activity agrees
up to that time with the continuation just defined. We also write~$L_T^P$ for
the event that the continuation of~$P$ has no successful interaction at any
time after~$T$.
...
\eqref{eq:local-continuation-probability} cancels the normalizer at length
~$\Delta+h$. Using
\begin{equation*}
\psi_i(\Delta+h,z)
=
...
On~$L_{T,t}$ the patch family at time~$t$ is determined by
~$\mathcal G_T$, so the definition of~$W_t^\mu$ gives the first equality in
