...
\begin{equation}
\label{eq:convergence-towards-m-star}
\liminf_{t\to\infty}
\inf_{\eta\in\Omega}
P_t\chi_A^\star(\eta)
\geq0
...
Fix a nonempty finite set~$A$ and choose~$I=\bar A$ and~$U=U(A)$ from the
definition of orientability. For~$\xi\in\cb{0,1}^U$, fixing the spins on
~$U$ at~$\xi$ defines a spin system on~$\cb{0,1}^I$. Write
~$P_t^{I,\xi}$ for its semigroup and
\begin{equation*}
\lambda_i^{I,\xi}(\eta)
=
\lambda_i\bb{\eta^{U,\xi}},
\qquad
\rho_i^{I,\xi}(\eta)
=
\rho_i\bb{\eta^{U,\xi}},
\qquad i\in I,
\end{equation*}
where~$\eta^{U,\xi}$ agrees with~$\eta$ on~$I$ and with~$\xi$ on~$U$.
The values outside~$I\cup U$ are irrelevant because~$I\cup U$ is
autonomous.
...
\begin{equation}
\label{eq:boundary-m-star-preservation}
\mathcal M_\star^I P_t^{I,\xi}
\subseteq
\mathcal M_\star^I
\qquad
\text{for every }t\geq0
\text{ and }\xi\in\cb{0,1}^U.
\end{equation}

We next show that, over every unit interval, a fixed positive part of the law
of the restricted process enters~$\mathcal M_\star^I$, uniformly over the
initial configuration and the boundary trajectory. Define
\begin{equation*}
\zeta_I(i)
=
\ind\bb{p_i^\star>0},
\qquad i\in I.
\end{equation*}
Then~$\delta_{\zeta_I}\in\mathcal M_\star^I$. Indeed, every factor in
~$\chi_B^\star(\zeta_I)$ is either~$1-p_i^\star\geq0$ or~$0$.

The pure-death component gives
\begin{equation*}
\lambda_i(\eta)\bb{1-\rho_i(\eta)}
\geq
\varepsilon
...
\text{for every }i,\eta,
\end{equation*}
and hence~$\lambda_i(\eta)\geq\varepsilon$. Moreover,
\eqref{eq:centered-generator-positive-coefficients} implies
\begin{equation*}
\lambda_i(\eta)\bb{\rho_i(\eta)-p_i^\star}
\geq0.
\end{equation*}
Thus, when~$p_i^\star>0$, the rate of an update setting site~$i$ to~$1$
satisfies
\begin{equation*}
...
\varepsilon p_i^\star,
\end{equation*}
while when~$p_i^\star=0$, the rate of an update setting~$i$ to~$0$ is at
least~$\varepsilon$. Since~$I$ is finite and the update rates are uniformly
bounded above, there is~$\alpha_I>0$ such that 
\begin{equation}
\label{eq:mass-leak-decomposition}
\delta_\eta P_{1}^{I,\xi}
=
\alpha_I\delta_{\zeta_I}
+
\bb{1-\alpha_I}\nu^{I,\xi,\eta}
\end{equation}
for some probability measure~$\nu^{I,\xi,\eta}$. The content of~\eqref{eq:mass-leak-decomposition} is that~$\alpha_I$ is uniform in~$\eta$ and boundary conditions~$\xi$. To see the uniform lower bound, one may prescribe disjoint
subintervals in which the sites of~$I$ are successively set to their
~$\zeta_I$ values and require no later update at sites already set; the lower
bounds above and the uniform upper bound on the total update rate give a
positive probability depending only on~$I$.

We now apply this minorization to the original process. Let
\begin{equation*}
\mathcal G_U
=
\sigma\cb{\eta_s(j):j\in U,\ s\geq0}.
\end{equation*}
Since both~$U$ and~$I\cup U$ are autonomous, conditional on
~$\mathcal G_U$ the process on~$I$ evolves with the revealed trajectory on
~$U$ as a time-dependent boundary condition. Only the spins in
\begin{equation*}
\partial I
=
N_*(I)\setminus I
\subseteq U
\end{equation*}
matter. This set is finite, and its trajectory has only finitely many jumps on
every bounded time interval. Hence the conditional transition on~$I$ is a
finite composition of semigroups~$P_s^{I,\xi}$.

For~$t\geq1$ and divide~$[0,t]$ into~$\lfloor t\rfloor$
blocks, the first~$\lfloor t\rfloor-1$ of length~$1$ and the last of length
~$t-{\lfloor t\rfloor}+1\geq1$. Denote the corresponding conditional transition operators on
~$I$ by~$Q_1,\ldots,Q_{\lfloor t\rfloor}$. Each~$Q_j$ is a finite composition of fixed-boundary
semigroups and therefore preserves~$\mathcal M_\star^I$ by
\eqref{eq:boundary-m-star-preservation}. Applying
\eqref{eq:mass-leak-decomposition} successively to the part of the law not
previously extracted gives
\begin{align*}
\CE{\chi_A^\star(\eta_t)}{\mathcal G_U}
&=
\sum_{k=1}^{\lfloor t\rfloor}
\alpha_I\bb{1-\alpha_I}^{k-1}
\bb{\delta_{\zeta_I}Q_{k+1}\cdots Q_{\lfloor t\rfloor}}
\bb{\chi_A^\star}
\\
&\quad+
\bb{1-\alpha_I}^{\lfloor t\rfloor}\nu_t(\chi_A^\star)
\end{align*}
for some probability measure~$\nu_t$, with the empty product interpreted as
the identity. Each term in the sum is nonnegative because
~$\delta_{\zeta_I}\in\mathcal M_\star^I$ and the~$Q_j$ preserve
~$\mathcal M_\star^I$. Therefore
\begin{equation*}
\CE{\chi_A^\star(\eta_t)}{\mathcal G_U}
\geq
-\norm{\chi_A^\star}_\infty
\bb{1-\alpha_I}^{\lfloor t\rfloor}.
\end{equation*}
Taking expectations proves
\eqref{eq:convergence-towards-m-star}.

It remains to identify the limit. Fix a local function~$f$ and
~$s>0$. Choose~$t_n\to\infty$ and~$\eta_n\in\Omega$ such that
\begin{equation*}
\abs{P_{t_n}f(\eta_n)-\pi(f)}
\longrightarrow
\limsup_{t\to\infty}
\sup_{\eta\in\Omega}
\abs{P_tf(\eta)-\pi(f)}.
...
\begin{equation*}
\delta_{\eta_n}P_{t_n-s}
\Rightarrow
\mu
\end{equation*}
for some probability measure~$\mu$, by compactness of the space of probability measures on~$\Omega$. For every
finite~$B\Subset\Lambda$,~\eqref{eq:convergence-towards-m-star} gives
\begin{equation*}
\mu(\chi_B^\star)
=
\lim_{n\to\infty}
P_{t_n-s}\chi_B^\star(\eta_n)
\geq0.
\end{equation*}
Hence~$\mu\in\mathcal M_\star$. Since~$P_sf$ is continuous by the Feller
property,
\begin{equation*}
P_{t_n}f(\eta_n)
=
\bb{\delta_{\eta_n}P_{t_n-s}}(P_sf)
\longrightarrow
(\mu P_s)(f).
\end{equation*}
Theorem~\ref{thm:common-invariant-limit} therefore gives
\begin{equation*}
\limsup_{t\to\infty}
\sup_{\eta\in\Omega}
\abs{P_tf(\eta)-\pi(f)}
\leq
K_f(1+s)^D e^{-\varepsilon s/2}.
\end{equation*}
Letting~$s\to\infty$ proves
\begin{equation*}
\lim_{t\to\infty}
\sup_{\eta\in\Omega}
\abs{P_tf(\eta)-\pi(f)}
