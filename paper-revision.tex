...
We relate the event that all successful interactions up to time~$T$ remain in
a finite set~$R$ to the spin system in~$R$ with zero boundary condition.

For~$R\subseteq\Lambda$, let~$\eta^{R,0}$ agree with~$\eta$ on~$R$ and be
zero on~$R^c$. Define
\begin{align*}
\lambda_i^R(\eta)
&=
...
We now identify the expectation on the right. Up to time~$T$, keep every death,
and keep a split or birth with nonempty target only when its source and all its
...
For~$i\in R$, the M\"obius expansions in
...
off-diagonal terms and subtracts the total rate of the retained jumps. Since
\begin{equation*}
V(B)-\kappa_R(B)
=
\sum_{i\in B}
...
\end{equation*}
the potential cancels this subtracted jump rate and restores the diagonal
coefficient~$a_i^\beta(\varnothing)$. Hence
\begin{equation}
...
The generator identity~\eqref{eq:confined-generator-duality} gives the
Feynman--Kac duality for the killed dual up to time~$T$. Combining it with the
