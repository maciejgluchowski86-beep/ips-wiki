...
~$\mathcal M_\star$ converges to~$\pi$. The point of orientability is that,
on every finite set, the evolution from an arbitrary configuration approaches
~$\mathcal M_\star$. More precisely, we will prove that
\begin{equation}
...
bounded above, there is~$\alpha_I>0$ such that every restricted evolution
over an interval of length at least~$1$, with arbitrary time-dependent boundary
condition on~$U$, satisfies
\begin{equation}
...
\bb{1-\alpha_I}\nu
\end{equation}
for some probability measure~$\nu$. Here~$Q$ denotes the transition operator
over that interval. To see the uniform lower bound, one may prescribe disjoint
...
For~$t\geq1$, let~$m=\lfloor t\rfloor$ and divide~$[0,t]$ into~$m$
blocks, the first~$m-1$ of length~$1$ and the last of length
~$t-m+1\geq1$. Denote the corresponding conditional transition operators on
~$I$ by~$Q_1,\ldots,Q_m$. Each~$Q_j$ is a finite composition of fixed-boundary
...
\bb{\delta_{\zeta_I}Q_{k+1}\cdots Q_m}
\bb{\chi_A^\star}
\\
&\quad+
\bb{1-\alpha_I}^m\nu_t(\chi_A^\star)
...
\end{equation*}
Taking expectations, uniformly in the initial configuration, proves
\eqref{eq:convergence-towards-m-star}.

It remains to identify the uniform limit. Fix a local function~$f$ and
...
\mu
\end{equation*}
for some probability measure~$\mu$, by compactness of~$\Omega$. For every
...
Finally, if~$\nu$ is invariant, then for every local~$f$,
\begin{equation*}
\abs{\nu(f)-\pi(f)}
=
\abs{(\nu P_t)(f)-\pi(f)}
...
\longrightarrow0.
\end{equation*}
Thus~$\nu=\pi$, and~$\pi$ is the unique invariant measure.
\end{proof}
