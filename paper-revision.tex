...
from Section~\ref{sec:dual}.
}
For fixed~$t\geq0$ and~$\eta\in\Omega$, write
\begin{equation*}
Z_t^\eta
=
\sigma_t
...
outgoing patch beginning at its source. {\color{red}Empty-target deaths have
...
z^{X_t^P}.
\end{equation*}
The preceding identities give the pathwise factorization
\begin{equation}
...
skeleton. In particular, empty-target deaths inside a patch are integrated out.
If a patch begins with an outgoing boundary, the skeleton fixes its
target~$S(P)$ but not whether that interaction was a split or a birth, so this
hidden kind and the sign it may carry are averaged under the consistent patch
law. When a split and a birth with the same target carry opposite signs, their
