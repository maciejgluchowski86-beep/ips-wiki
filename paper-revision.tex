...
\textcolor{red}{\textbf{Revision note.} This restored appendix still refers
to the old label \texttt{eq:rate-expansion}; the corresponding current label
is \texttt{eq:rate-expansion-revision}. This should be updated when the
appendix is revised.}
...
\textcolor{red}{\textbf{Revision note.} Section~\ref{sec:dual} now defines the
dual rates, signs, and potential in the main text. This subsection should be
shortened to the generator verification and the finite-to-infinite-volume
Feynman--Kac passage, avoiding repetition of material already explained there.}
...
\textcolor{red}{\textbf{Revision note.} The formulas below are still used
throughout Section~\ref{sec:proofs}. The heading and surrounding terminology
should be aligned with the revised paper, which now distinguishes bulk patches
from infinite patches explicitly.}

\subsection{Contributions without terminal variables}
...
In terms of the M\"obius coefficients of the spin-system parameters, the
...
outgoing mark before the endpoint, or when the first outgoing mark is a death
mark with~$S=\varnothing$. Therefore
\begin{equation*}
\pP_P\bb{\operatorname{Con}(P)}
=
e^{-\alpha_i\Delta}
...
\textcolor{red}{\textbf{Revision note.} This restored appendix also contains
references to the old label \texttt{eq:rate-expansion}; these should be
changed to \texttt{eq:rate-expansion-revision} in the revised version.}

\textcolor{red}{\textbf{Revision note.} Most of this subsection now duplicates
Lemma~\ref{lem:dual-local-finiteness} in Section~\ref{sec:patches}, including
the same label. It should not remain twice. The revised version should retain
only the extra consequences used later, in particular that backward predecessor
chains reach time~$0$ and that a finite patch family is equivalent to having a
...
successful-interaction skeleton and the patch family.
\end{proof}
...
\end{equation*}
Write~$P_t^R$ for its semigroup. Inside~$R$, this is the original spin system
with the spins outside~$R$ fixed at~$0$. Outside~$R$, the update rate and refresh
parameter are constant, so those sites evolve independently. Hence, if~$f$
depends only on spins in~$R$,
\begin{equation}
...
During the dual interval~$[0,T]$, retain every death interaction with
$S=\varnothing$ and retain a nonempty-target split or birth only when its source
and all its targets lie in~$R$. Let~$\Dcal_R$ be the resulting signed-dual
generator. If the active set is~$B$, the total rate of discarded interactions is
\begin{equation*}
\kappa_R(B)
=
\sum_{i\in B}
...
For~$i\in R$, the M\"obius expansions in~\eqref{eq:rate-expansion} give
\begin{align*}
...
\end{align*}
Thus, for~$Y=(B,\sigma)$ with~$B\subseteq R$, the generator calculation from
Appendix~\ref{app:generator-action} gives
\begin{align}
\Lcal_\eta^R H(Y,\eta)
={}&
\sigma
...
probability and using~\eqref{eq:all-zero-neighbourhood-relaxation} gives
\begin{equation}
...
\prod_{P\in\mathcal E_T}b(P).
\end{equation*}
The two expressions compared in the lemma are both at least
$\prod_Pa_t(P)$, so the sum of the two preceding errors bounds their absolute
difference.
\end{proof}

\textcolor{red}{\textbf{Revision note.} The finite-propagation lemma is still
needed by Section~\ref{subsec:proof-common-invariant-limit}. If the duplicated
local-finiteness subsection above is removed, this subsection must define
~$\overline\lambda$ locally rather than relying on its earlier definition.}
...
\begin{proof}
Construct the process from rate-$\overline\lambda$ clocks at each site,
together with independent marks implementing the state-dependent acceptance
and refresh. Explore these clocks backwards from~$A$ at time~$t$. Whenever a
clock at~$i$ is encountered, add~$N(i)\cup\cb{i}$ to the sites whose states at
earlier times must be determined. If this backward influence cluster stays
...
at least~$n$ clocks. There are at most~$\abs A m^k$ spatial paths of length~$k$,
and the expected number of chronologically ordered~$k$-tuples along a fixed
path is~$(\overline\lambda t)^k/k!$. Therefore, for every~$\theta>0$,
\begin{equation*}
\pP\bb{\text{the influence cluster leaves }B(A,n)}
\leq
\abs A\sum_{k\geq n}\frac{\bb{m\overline\lambda t}^k}{k!}
\leq
\abs A\exp\bb{m\overline\lambda t e^\theta-\theta n}.
\end{equation*}
Take~$n=\lceil vt\rceil$ and choose~$v$ so that
$\theta v-m\overline\lambda e^\theta>a$. After increasing the constant to cover
...
\textcolor{red}{\textbf{Revision note.} The following paragraph refers to the
obsolete label \texttt{thm:patch-positivity-order} and to an earlier
finite-volume proof of order preservation. The current proof of
Theorem~\ref{thm:centered-monomial-monotonicity} is different, so this
paragraph should be deleted unless a finite-volume approximation is restored
there.}
...
\textcolor{red}{\textbf{Revision note.} This subsection is still needed for
Section~\ref{subsec:proof-orientability}, but its proof needs revision. It uses
the old notation~$\mathcal M_*^I$,~$\chi_B^*$ and the obsolete labels
\texttt{eq:principal-coefficient-criterion} and
\texttt{eq:principal-threshold-profile}. It should instead use the current
centered-monomial notation and the coefficient inequalities proved in
Section~\ref{subsec:proof-centered-monomial-monotonicity}. The prose also
contains several typographical errors.}

\subsection{Restricted system preserves~$\mathcal{M}_*^I$}
...
$U\subseteq\Lambda\setminus I$ satisfy~$N(i)\subseteq I\cup U$ for every
$i\in I$. For~$\xi\in\cb{0,1}^U$, let~$P_t^{I,\xi}$ be the semigroup on
$\cb{0,1}^I$ obtained by fixing the spins on~$U$ at~$\xi$.
Let~$\mathcal M_*^I$ consist of probability measures on~$\cb{0,1}^I$
whose moments centered at~$(p_i^\star)_{i\in I}$ are
nonnegative. Then
\begin{equation*}
...
\begin{proof}
The proof is by demonstration that the expension of~$\mathcal{L}^{I,\xi}\bb{\chi_B^*}$ in centered moments has non-negative off-diagonal coefficients.  First observe, that for~$S\subseteq N(i)\cap I$ we have
\begin{align*}
\widehat{\lambda_i^{I,\xi}}(S)
&=\sum_{T\subseteq N(i)\cap U}
 \widehat{\lambda_i}(S\cup T)\prod_{j\in T}\xi(j)
\leq0,
\quad(S\neq\vn),\\
\widehat{\lambda_i^{I,\xi}(\rho_i^{I,\xi}-p_i^\star)}(S)
&=\sum_{T\subseteq N(i)\cap U}
 \widehat{\lambda_i(\rho_i-p_i^\star)}(S\cup T)
 \prod_{j\in T}\xi(j)
\geq0.
\end{align*}
The inequalities hold because each term with~$S\cup T\neq \vn$ in  the sums has the right sign, see~\eqref{eq:principal-coefficient-criterion} and
\eqref{eq:principal-threshold-profile}. The second sum also includes the term corresponding to~$S\cup T = \vn$, which is
$\lambda_i(\mb0)(\rho_i(\mb0)-p_i^\star)\geq0$, see remark under~\eqref{eq:principal-threshold-profile}.
Thus both~$\lambda_i^{I,\xi}(\rho_i^{I,\xi}-p_i^\star)$ and
$\lambda_i^{I,\xi}(\mb0)-\lambda_i^{I,\xi}$ have nonnegative monomial
coefficients.

Choose~$h>0$ with~$h\sum_{i\in I}\lambda_i^{I,\xi}(\mb0)\leq1$.
For~$B\subseteq I$,
\begin{align*}
(\mathrm{Id}+h\Lcal^{I,\xi})\chi_B^*
&=\bb{1-h\sum_{i\in B}\lambda_i^{I,\xi}(\mb0)}\chi_B^*\\
&\quad+h\sum_{i\in B}
 \lambda_i^{I,\xi}(\rho_i^{I,\xi}-p_i^\star)
 \chi_{B\setminus\cb{i}}^*\\
&\quad+h\sum_{i\in B}
 \bb{\lambda_i^{I,\xi}(\mb0)-\lambda_i^{I,\xi}}\chi_B^*.
\end{align*}
The identities
\begin{equation*}
\eta(j)=p_j^\star+\chi_{\cb{j}}^*,
\qquad
\eta(j)\chi_{\cb{j}}^*
=p_j^\star(1-p_j^\star)+(1-p_j^\star)\chi_{\cb{j}}^*
\end{equation*}
show that a product of a monomial and a centered monomial can be represented by a linear combination of centered monomials with only non-negative coefficients. Hence so can
$\bb{\mathrm{Id}+h\Lcal^{I,\xi}}^n\bb{\chi_B^*}$ for any~$n\in\N$. Since~$I$ is finite, for~$\mu\in\mathcal M_*^I$,
\begin{equation*}
(\mu P_t^{I,\xi})(\chi_B^*)
=\lim_{n\to\infty}
 \mu\bb{\bb{\mathrm{Id}+\frac{t}{n}\Lcal^{I,\xi}}^n\chi_B^*}
\geq0
\qquad(B\subseteq I).
\end{equation*}
\end{proof}
