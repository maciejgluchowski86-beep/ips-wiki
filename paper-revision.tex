...
spin systems in the signed form needed for the patch representation; see~\cite{Harris1978,Griffeath1979,Gray1986,JansenKurt2014}. We identify the
...
values, while their signs are carried by the second coordinate. For~$S\subseteq N(i)$, set
\begin{equation}
...
Under this condition, the generator identity~\eqref{eq:monomial-generator-duality} gives the standard Feynman--Kac
duality relation
...
\end{equation}
\end{theorem}
This is a standard Feynman--Kac duality argument; see, for example,~\cite{JansenKurt2014}.
...
\end{align*}
The formal record~$(\infty,0,A)$ implies that~$0\in\mathcal T^{\mathsf I}(i)$, meaning we interpret the initial conditions of the process as an incoming successful interaction.
...
before it, and would create an additional successful-interaction cutting the patch into smaller patches. Hence such marks can only occur with~$X_{u-}^P=0$. If the terminal boundary is outgoing, the recorded interaction
...
relative to the system with the pure deaths removed, by a factor~$e^{-\varepsilon u}$. Since the latter provides an integrable upper bound,
the contribution from the event that a successful interaction occurs
after~$T$ is of order~$e^{-\varepsilon T}$.

It remains to consider the event that there is no successful interaction after~$T$ and show that on this event the dependence on the initial law decays. The
...
on~$z$ decays at rate at least~$\varepsilon$, and hence is of order~$e^{-\varepsilon(t-T)}$. This is the mechanism by which the dependence on the
...
same dual process and thus the same successful-interaction skeleton and patch families, and for~$\mu\in\mathcal M_\star$,
\begin{equation}
...
Fix the successful-interaction skeleton up to time~$T$. For~$P\in\mathcal E_T$, let~$L_{T,t}^P$ be the event that the one-site
...
\bb{z-\rho_i(\mb 0)}.
\end{equation*}
Since the system contains a pure-death component of rate~$\varepsilon$,~$\lambda_i(\mb 0)\geq\varepsilon$. As~$C(z,P)$ is affine in~$z$,
the factor in
\eqref{eq:main-local-end-relaxation} relaxes to~$C(\rho_i(\mb 0),P)$ at rate at least~$e^{-\varepsilon(t-T)}$.
...
C\bb{\rho_{i(P)}(\mb 0),P}.
\end{align*}
The event~$E_T^R$ is~$\mathcal G_T$-measurable. For the first term,~$B_T\leq B_T^\varepsilon$, and we get
\begin{align*}
&\E_A\Cb{
 \ind\bb{E_T^R}
 B_T
...
Apply Lemma~\ref{lem:finite-propagation} with~$a=\varepsilon$. There are~$v<\infty$ and~$C_A<\infty$ such that, for
\begin{equation*}
R_T
=
...
K_f(1+t)^D e^{-\varepsilon t/2}
\end{equation*}
proves~\eqref{eq:uniform-m-minus-convergence}. Since~$\mathcal M_- = \bigcup_{K\geq0}\mathcal M_{-,K}$, it also follows that
\begin{equation*}
\mu P_t\Rightarrow\pi
\qquad
\text{for every }\mu\in\mathcal M_-.
...
that every configuration with finitely many~$0$s gives an initial law in~$\mathcal M_-$. The pure-death component gives
\begin{equation*}
...
\end{equation*}
Combining this with~\eqref{eq:confined-weight-to-dual} and integrating over~$\mu$ proves~\eqref{eq:confined-interaction-identity}.
\end{proof}
...
\CP{L_{T,t}^P}{\mathcal G_T}.
\end{equation}
Indeed, each~$L_{T,t}^P$ implies that there is no successful interaction at~$i(P)$ during~$[T,t)$, so~$L_{T,t}$ must lie in the intersection of these events. The opposite inclusion is trivial. The second identity follows because distinct end patches are based at distinct sites, so conditional on~$\mathcal G_T$ the events~$(L_{T,t}^P)_{P\in\mathcal E_T}$ are independent.
...
Suppose that~$\Lcal$ is patch-positive. Let~$I\Subset\Lambda$ and~$U\subseteq\Lambda\setminus I$ satisfy
\begin{equation*}
N(i)\subseteq I\cup U
\qquad
\text{for every }i\in I.
\end{equation*}
For~$\xi\in\cb{0,1}^U$, let~$P_t^{I,\xi}$ be the semigroup on~$\cb{0,1}^I$ obtained by fixing the spins on~$U$ at~$\xi$. Define
\begin{equation*}
\mathcal M_\star^I
=
\cb{
...
\eqref{eq:restricted-centered-sign}, all coefficients in~$(I+r\Lcal^{I,\xi})\chi_B^\star$ are nonnegative before the products~$\chi_S\chi_D^\star$ are expanded. Equation~\eqref{eq:ordinary-centered-product-expansion} shows that each such product
